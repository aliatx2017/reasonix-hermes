...
- **Production is a tag, not a branch.** A release is a tagged snapshot of `main-v2`:
  `v1.9.x` (CLI), `hermes-npm-v1.9.x` (npm), `desktop-v1.9.x` (desktop).
...
5. **Ship stable** when the canary is clean — push the three tags:
   ```sh
   git tag v1.9.0         && git push origin v1.9.0          # CLI binaries + Homebrew
   git tag hermes-npm-v1.9.0     && git push origin hermes-npm-v1.9.0      # npm -> next
   git tag desktop-v1.9.0 && git push origin desktop-v1.9.0  # desktop -> R2 latest/
   ```
...
6. **Promote to default install** (optional, when 1.x should become the bare `npm i` target):
   ```sh
   npm dist-tag add reasonix-hermes@1.9.0 latest
   ```
...
- A stable `-rc` tag (e.g. `hermes-npm-v1.9.0-rc.1`) still ships under `next`, not `canary`.
