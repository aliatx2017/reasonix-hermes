...
- **fix(control) `40c4bfcc`** — `SendCtx` discarded its context and returned nil unconditionally, and the `Send` path underneath reached `runGuarded`, which returns silently when `c.running` is already true. A cron task firing during an active turn therefore vanished while `runTask` recorded `Success: true, Summary: "task dispatched"`, and the scheduler's 10-minute per-task deadline bounded nothing. `runGuarded` is now a thin wrapper over `startGuarded`, which keeps the same lifecycle (autosave, panic recovery, closing `TurnDone` event) but derives the turn context from a caller-supplied parent, returns `ErrTurnRunning` instead of returning silently, and hands back a buffered channel carrying the turn's error. `SendCtx` waits on that channel, so a dropped task surfaces as an error and the caller's deadline cancels the turn. Both callers wanted the synchronous form: the scheduler records the outcome, and `cmd/learner-live-test` no longer polls `Running()`, which raced against the turn goroutine setting it. Due tasks now run one at a time — the agent runs a single turn at a time, so dispatching them together would only make all but one fail.
  - This item was recorded as blocked in the previous session by a local tooling failure that rejected every write to `internal/control/controller.go`. The file edited normally in a fresh session after the four `PreToolUse` hooks were removed from the shared ECC plugin cache; the failure was never reproducible outside the harness, so the hook removal is correlation, not a confirmed root cause.
