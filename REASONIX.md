...
- **Upstream synced**: `v1.10.0` (commit f2a475a2, 2026-06-22). 32 syncs total — merged: f2a475a2 (h55, 28 commits). Previous sync: 043e6183 (h54, 10 commits).
...
- **Commit**: session 2026-06-22 (h55) — upstream merge (32nd sync) + doc-sweep (22 claims). **Upstream**: merged f2a475a2 (28 commits — scroll reliability, image-path containment, grace round on tool-call budget, crash stats 30d, /reload-cmd, lazy MCP schema cache, transcript reveal fixes, model-switcher compact, session sidecars with trash, workspace vision images). 10 conflicts resolved across Go + TS + lockfiles. **refs.go**: Removed Hermes image-fallthrough (classifyRef isImageExtension, detectRefsMode non-workspace image pass-through) — aligned with upstream's stricter os.OpenRoot path containment. **agent.go**: Merged graceRound loop condition + upstream budget-exhausted nudge message while preserving Hermes learner outcomes tracking and compress.SetTurn. **go.mod**: go mod tidy restored transitive deps (dustin/go-humanize, x/crypto). **package.json**: d3 + @types/d3 preserved. **useScrollManager.ts**: Accepted upstream useState + onComplete. **Doc-sweep**: 22 stale claims fixed across 10 docs — SPEC.md (7: line counts, sub-file descriptions, builtin tools 9→20, v1.8.1→v1.11.0, memory/SQLite), README.md (5: MCP/memory tool name prefixes, 19→17 features, 20+→19 sections, 125+→59 skills), README.zh-CN.md (2), HERMES-GUIDE.md (4: v1.10→v1.11 ×2, broken OLLAMACLOUD.md→§16.13, missing binaries, cross-compile note), BOT_GUIDE en+zh-CN (QQ added to intro/usage/diagram), SKILLS-CATALOG (42 project/active), CONFIG_PATHS.zh-CN + GUIDE en+zh-CN + MIGRATING (v1.8.1→v1.11.0 ×9), CHANGELOG-HERMES (h55 entry). **Build**: All 9 binaries rebuilt (Jun 22). Full test suite (76 packages) + go vet + tsc + fingerprint guards 34/34 all pass. 3 commits pushed. Go-review + go-test + golang-patterns audit all clean.
...
- **Commit**: session 2026-06-21 (h50) — coding-standards audit, doc-sweep, upstream sync. 70 silent `.catch(() => {})`→console.warn across 27 desktop files. 10 any holes typed in bridge.ts. Doc-sweep: 102 docs, 12 stale claims fixed. SPEC.md +pkg/ tree. QQ bot section in BOT_GUIDE (EN+ZH-CN). i18n verified (335 keys×3). 38 files changed. All 9 binaries rebuilt. Full test suite + tsc clean.
- **Commit**: session 2026-06-20 (h49) — LobeHub API migration, AnalyticsPanel nil-slice fix, doc-sweep. LobeHub: endpoint /api/v1/skills→/api/v1/agents (M2M tokens rejected by skills endpoint), SkillListResponse→AgentListResponse, LobeHubSkillItem→LobeHubAgentItem, pageSize capped at 20, CJK-heavy entries filtered (>30% CJK = skip), singleton DefaultRegistry (sync.Once) with RWMutex for concurrent desktop polling. AnalyticsPanel: TurnTimeline ToolCalls nil→[]string{} via orEmpty(), 3 null-safe guards in TSX. Doc-sweep: 14 stale claims fixed across 6 docs (binary sizes + learner-live-test, '360k+→856 agents', upstream sync commit 49c1476→91fe06d). Upstream merged (027e7dc0, 4 commits — MCP startup retry, lazy mode removal). 2 conflicts resolved. All 9 binaries rebuilt. Full test suite + race detection + tsc clean. 14 files changed.
