...
- **Commit**: session 2026-06-20 (h48) — code cleanup, MCP registrations, taste-skill commit, doc-sweep. 4 commits total. Code: removed 3 dead code items (ansiLogoIndigo/Cyan/Pink, Compressor.evictLocked, Controller.beep + connectCodegraphMCPServer), fixed 17 ignored-error sites, fixed 4 empty branches, replaced 52 WriteString(fmt.Sprintf(...)) → fmt.Fprintf across 9 files. 2 preallocation fixes (golang-patterns audit). 1 gofmt indentation fix (slack/slack.go). MCP: headroom v0.26.0 + markitdown-mcp v0.1.5 registered as stdio plugins in reasonix.toml. Taste-skill: 13 design-direction SKILL.md files committed. Doc-sweep: i18n key count 332→333, acp added to AGENTS.md customizations table. 41 files, +6,978/-127. All 9 binaries rebuilt. Full test suite + race detection + tsc clean.
