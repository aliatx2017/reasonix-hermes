...
- **Commit**: session 2026-06-20 (h46) — upstream sync (91fe06d, 4 commits: MiMo built-ins migration, codebase-memory MCP auto-indexing). 4 conflicts resolved: README.md/README.zh-CN.md (upstream bullet-point foundation), desktop/settings_app_test.go (MiMo test), internal/config/config.go (deleted Hermes MiMo backfill functions). 1 upstream regression fixed: normalizeLegacyDesktopProviderAccessForSettings data loss (SaveTo with RenderScopeProject stripped [desktop] section from project config, causing TestSettingsSeedsMissingUserConfigFromLegacyProjectConfig failure — also fails on clean upstream checkout). All 9 binaries rebuilt. All tests pass (main + desktop). tsc clean.
