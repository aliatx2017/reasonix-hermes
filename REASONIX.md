...
- **Upstream synced**: `v1.10.0` (commit 7aa97d31, 2026-06-22). 33 syncs total — merged: 7aa97d31 (h56, 28 commits). Previous sync: f2a475a2 (h55, 28 commits).
...
- **Commit**: session 2026-06-22 (h55) — upstream merge (32nd sync) + doc-sweep (22 claims).

- **Commit**: session 2026-06-22 (h56) — upstream merge (33rd sync, 7aa97d31, 28 commits) + headroom launchd daemon. **Upstream**: merged 7aa97d31 (28 commits — desktop MCP headers, remote MCP header support, rewind/plan prompt consolidation, incremental project config writes, session deletion perf, skill root dedupe, ACP config switch hardening, ask-user decision integration, short choice planner context, creation tool card scrollbar, CLI session migration for desktop, post-merge bot review fixes). 4 conflicts resolved: **package.json** — accepted upstream list + restored hotbar-guard.test.ts, d3, @codemirror/*, @biomejs/biome deps. **useController.ts** — kept Hermes console.warn catch handlers (silent-catch fix). **boot.go** — added config.UserDecisionPolicy injection alongside our languagePolicy(). **config.go** — shortened DefaultSystemPrompt (ask-tool text extracted to UserDecisionPolicy constant), added UserDecisionPolicy constant. **Headroom**: launchd daemon configured (.reasonix/headroom/com.headroom.proxy.plist), proxy v0.26.0 verified healthy, docs/HEADROOM.md updated with corrected flags and plist reference. **Build**: All 9 binaries rebuilt. Full test suite (76 packages) + go vet + tsc + fingerprint guards 34/34 all pass.
