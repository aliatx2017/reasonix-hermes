...
- **2026-08-24 session** (deferred subsystem sweep — 4 of 5 items fixed, 5 commits, no release):
 ...
 - **Still open — scheduler drop-but-report-success**: diagnosed, not fixed. `SendCtx` discards its ctx and always returns nil; `Send` → `runGuarded` returns immediately when `c.running`, so a cron task firing mid-turn vanishes while `runTask` records `Success: true, Summary: "task dispatched"`, and the 10-min timeout binds nothing. Fix requires splitting `runGuarded` in `internal/control/controller.go` into a shared `startGuarded` plus a synchronous variant returning `ErrTurnRunning`; that file could not be written this session due to a local tooling failure.
