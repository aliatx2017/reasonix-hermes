...
export const zh: Record<DictKey, string> = {
  ...
  'settings.agentRuntimeHint':
    '保存为用户级默认值。项目的 ./reasonix.toml 可以覆盖；设为 0 表示不限。',
  'settings.executorMaxSteps': '执行轮数上限',
  'settings.executorMaxStepsHint':
    '限制主 agent 每轮回复最多调用多少轮工具。复杂代码任务适合不限。',
  ...
  'settings.botInstallManualQQ':
    'QQ 当前仍使用官方 Bot App ID / Secret 手动配置，请在高级设置里填写。',
  ...
  'settings.telemetryHint':
    '启动时发送随机安装 ID、版本号和操作系统用于统计活跃安装量；绝不包含对话、密钥或文件内容。',
  'settings.metricsLabel': '共享聚合质量指标',
  'settings.metricsHint':
    '默认关闭。开启后发送匿名的轮次结束统计（结束原因、缓存命中率、错误与工具失败的类别），用于跨版本发现问题。只含枚举化计数——绝不包含对话、提示词、密钥、路径或任何文本。',
  ...
  'onboarding.tagline':
    '粘贴一个 DeepSeek API key 即可开始。密钥仅存于本应用的本地凭据文件，不会发往任何地方。',
