...
export const zh: Record<DictKey, string> = {
  ...
  'caps.envPreserveHint':
    '为保护密钥，已有变量值不会回显；留空会保留现有环境变量，填写后会整体替换。',
  'caps.remove': '删除服务器',
  ...
  'memory.autoSuggestionsHint':
    '开启后进入建议页会自动扫描近期本地历史；候选仍需手动确认后才会保存。',
  ...
  'memory.suggestionsEmptyBody':
    '扫描近期会话，找出可能值得保存的记忆和重复出现的工作流。结果只作为草稿展示。',
  ...
  'settings.capabilitiesDescription':
    '在专用管理面板中管理 MCP 服务器、授权、启用状态、重试和 Agent 技能。',
  ...
  'settings.subagentHint':
    '作用于 task 和 runAs=subagent skills；工具调用参数或单个 skill 配置会覆盖它。',
  ...
  'settings.coldResumePruneHint':
    '重开过期会话时精简旧工具结果，降低续聊成本；关闭则保留完整历史。',
  ...
  'settings.providerReasoningProtocolHint':
    '用于 OpenAI-compatible 推理模型。deepseek 使用 DeepSeek reasoning 参数，openai 使用普通 reasoning_effort，none 表示禁用。',
  'settings.supportedEfforts': '支持的 effort 等级',
  'settings.supportedEffortsCustomPlaceholder': '自定义等级（按回车）',
  'settings.supportedEffortsHint':
    '勾选该模型服务在 /effort 命令中暴露的等级，或添加自定义名称。留空则使用内置默认（DeepSeek：high|max，Anthropic：low|medium|high|xhigh|max）。',
  'settings.defaultEffort': '默认 effort',
  'settings.defaultEffortAuto': 'auto（使用内置默认）',
  'settings.defaultEffortHint':
    '在 /effort 为 auto 或未设置时使用。必须是上方等级之一；为空时回退到第一个等级（无选中时回退到内置默认）。',
  ...
  'settings.botBrowserPreviewWarning':
    '当前是浏览器预览，只会保存 mock 设置，不会写入本机配置或启动 QQ/IM Bot gateway。请在桌面应用或 CLI 中配置后再到 QQ 客户端测试。',
  ...
  'settings.botToolApprovalModeHint':
    '控制该渠道是否在工具执行前请求确认；计划确认和 ask 问题仍会等待回复。',
  ...
  'settings.botQQAccessRequired':
    '启用 QQ Bot 前，请先在高级设置里添加 QQ 用户/群白名单，或临时改为允许所有人。',
  ...
  'settings.yoloHint':
    'Yolo批准会跳过所有工具权限审批（写文件、bash）。deny 规则仍会拦截，ask 问题和计划确认仍会等待你选择。仅运行时生效，重启后失效。',
  ...
  'settings.style.hermes.note': '金辉',
  'settings.style.hermes.desc': '双蛇杖金辉强调色,温润暗色表面搭配青绿点缀。',
  ...
  'settings.fontFamilyCustomPlaceholder':
    '已安装字体或字体栈，如 "LXGW WenKai", "Microsoft YaHei", sans-serif',
  ...
  'settings.monoFontFamilyCustomPlaceholder':
    '等宽字体栈，如 "Maple Mono", "Cascadia Code", monospace',
  ...
  'settings.customProvidersHint':
    '你自己添加的 OpenAI-compatible 或代理接入，可以编辑、删除并持久化模型列表。',
  'settings.customProvidersEmpty': '还没有自定义供应商。常用服务优先使用上方内置接入。',
  'settings.providerAccessEmptyHint':
    '点击“添加供应商”，选择 DeepSeek、Mimo 或自定义 OpenAI-compatible 接入。',
  ...
  'settings.addProvider.officialHint':
    '官方预设会写入固定端点和默认参数，只需要选择供应商并配置密钥。',
  ...
  'settings.addProvider.official.mimoApiDesc':
    '适合按量 API 计费用户，包含 mimo-v2.5-pro 以及可识图的 mimo-v2.5 / mimo-v2-omni。',
  'settings.addProvider.official.mimoTokenPlan': 'Mimo Token Plan',
  'settings.addProvider.official.mimoTokenPlanDesc':
    '适合 Token Plan 订阅用户，使用专属接入地址，包含 pro 与可识图的 mimo-v2.5。',
  ...
  'settings.providerApiKeyEnvHint':
    '默认会根据供应商名称自动生成，只有需要固定环境变量名时再修改。',
  ...
  'settings.fetchModelsAfterKeyFailedForProvider':
    '密钥已保存，但暂时无法刷新 {provider} 的可用模型：{err}',
  ...
  'settings.reasoningProtocolHint':
    '控制 effort 的请求格式。Auto 会按模型能力表和官方端点识别；代理不兼容时可切到 OpenAI 或 None。',
  ...
  'settings.hooksScopeHint':
    'Hooks 会执行 shell 命令。全局 hooks 属于你本人；项目 hooks 必须显式信任后才会加载。',
  ...
  'settings.hooksJsonHint':
    '直接编辑 settings.json 中的 {"hooks": ...}。也支持粘贴带 event 字段的 hook 数组；保存前会校验并格式化。',
  ...
  'settings.hookMatchHint':
    '仅 PreToolUse 和 PostToolUse 使用这个锚定正则。匹配 read_file 需要写 .*file。',
  ...
  'settings.telemetryHint':
    '启动时发送随机安装 ID、版本号和操作系统用于统计活跃安装量；若上次运行发生崩溃，则在本次启动补发该崩溃报告。绝不包含对话、密钥或文件内容。',
  'settings.metricsLabel': '共享聚合质量指标',
  'settings.metricsHint':
    '默认开启。发送匿名的轮次结束统计和设置偏好快照（如桌面风格、主题、Provider/模型标识、客户端版本与机器人开关），以及用于 DAU 去重的随机安装 ID。Bucket 可能包含归一化后的自定义 Provider 名和模型名——绝不包含对话、提示词、密钥、路径、base URL 或文件内容。',
  ...
  'onboarding.tagline':
    '粘贴一个 DeepSeek API key 即可开始。密钥仅存于本机凭据存储，不会发往任何地方。',
  ...
  'performanceReport.privacyNote':
    '上传前桌面端会移除路径与密钥；诊断信息只用于记录耗时、内存、网络状态、近期 breadcrumbs、应用版本和操作系统。',
