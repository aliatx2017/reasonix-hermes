...
eq(
  userTimeItems[0]?.kind === 'user' && userTimeItems[0].createdAt,
  undefined,
  'history users without createdAt keep no timestamp',
);
eq(
  userTimeItems[1]?.kind === 'user' && userTimeItems[1].createdAt,
  1_718_000_000_000,
  'history users preserve createdAt when present',
);
    ...
    toolCalls: [
      {
        id: 'todo-bad',
        name: 'todo_write',
        arguments: '{"todos":[{"content":"Bad","status":"in_progress"}]}',
      },
    ],
  },
  {
...
eq(
  lowercaseError[0]?.kind === 'tool' && lowercaseError[0].status,
  'error',
  'lowercase error result restores as error',
);
eq(
  Boolean(lowercaseError[0]?.kind === 'tool' && lowercaseError[0].error),
  true,
  'lowercase error result carries error text',
);
...
eq(
  missingResult[0]?.kind === 'tool' && missingResult[0].status,
  'stopped',
  'missing tool result restores as stopped',
);
...
eq(
  positionalResult.length,
  1,
  'positional tool result is consumed instead of rendering as an orphan',
);
eq(
  positionalResult[0]?.kind === 'tool' && positionalResult[0].status,
  'done',
  'empty-id tool call restores from positional result',
);
eq(
  positionalResult[0]?.kind === 'tool' && positionalResult[0].output,
  'Todos updated',
  'empty-id tool call keeps positional output',
);
