...

{
  ...
  eq(
    displayedComposerProfileCollaborationMode(profiles['tab-1']),
    'plan',
    'stale tab hydration keeps locally selected plan mode',
  );
  eq(
    profiles['tab-1'].tokenMode,
    'economy',
    'token saver remains independent of collaboration mode changes',
  );
  eq(composerProfileMode(profiles['tab-1']), 'plan', 'compat mode keeps the plan axis enabled');
  eq(
    Boolean(profiles['tab-1'].pending.collaborationMode),
    true,
    'pending plan stays pending until backend acknowledges it',
  );

  profiles = hydrateComposerProfilesFromTabs(profiles, [
    tab({ mode: 'plan', collaborationMode: 'plan', tokenMode: 'economy' }),
  ]);

  eq(
    displayedComposerProfileCollaborationMode(profiles['tab-1']),
    'plan',
    'acknowledged tab hydration keeps plan visible',
  );
  eq(
    Boolean(profiles['tab-1'].pending.collaborationMode),
    false,
    'backend acknowledgement clears pending plan',
  );
}

{
  ...
  profiles = patchComposerProfile(profiles, 'tab-1', profiles['tab-1'], { tokenMode: 'economy' }, [
    'tokenMode',
  ]);
  ...
  eq(
    profiles['tab-1'].tokenMode,
    'economy',
    'stale meta cannot erase a pending token saver selection',
  );
  eq(
    Boolean(profiles['tab-1'].pending.tokenMode),
    true,
    'token saver stays pending while meta is stale',
  );
  ...
  eq(
    Boolean(profiles['tab-1'].pending.tokenMode),
    false,
    'token saver pending clears after matching meta',
  );
}

{
  ...
  eq(
    displayedComposerProfileCollaborationMode(profiles['tab-1']),
    'goal',
    'empty goal draft remains visible through stale tab hydration',
  );
  eq(
    controllerComposerProfileCollaborationMode(profiles['tab-1']),
    'normal',
    'empty goal draft syncs to controller as normal',
  );
  eq(
    composerProfileMode(profiles['tab-1']),
    'normal',
    'empty goal draft does not enable plan compatibility mode',
  );
}

{
  ...
  profiles = patchComposerProfile(profiles, 'tab-2', profiles['tab-2'], { tokenMode: 'economy' }, [
    'tokenMode',
  ]);
  profiles = hydrateComposerProfilesFromTabs(profiles, [tab()]);
