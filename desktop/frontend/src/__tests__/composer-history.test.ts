...
async function testSnapshotSupportsTupleResult() {
  setMock(async () => [
    [{ text: 'tuple 1', at: Date.now(), sessionPath: '/mock/a.jsonl', turn: 0 }],
    'tuple-nonce',
  ]);
  const entries = await snapshot();
...
async function testTupleCacheHitRespectsTupleNonce() {
  ...
  setMock(async (nonce) => {
    ...
    return [
      [{ text: 'tuple cache', at: Date.now(), sessionPath: '/mock/a.jsonl', turn: 0 }],
      'tuple-hit',
    ];
  });
...
async function testInvalidateResetsNonce() {
  ...
  setMock(async (request) => {
    ...
    return {
      entries: [{ text: 'msg', at: Date.now(), sessionPath: '/mock/a.jsonl', turn: 0 }],
      nonce: 'server-nonce',
      olderCursor: '',
      hasOlder: false,
    };
