...
for (const selector of ['.palette__title', '.palette__hint']) {
  ...
  eq(
    finalDeclaration(selector, 'text-overflow'),
    'ellipsis',
    `${selector} uses ellipsis for long text`,
  );
  eq(finalDeclaration(selector, 'white-space'), 'nowrap', `${selector} stays on one measured line`);
}

ok(
  !matchingBlocks('.palette__item').some((block) =>
    /(?:^|;)\s*flex-direction\s*:\s*column\s*(?:;|$)/.test(block),
  ),
