...
const cases: BenchCase[] = [
  ...
  {
    name: '1000-turns-archived-3-tools',
    turns: 1000,
    toolsPerTurn: 3,
    outputSize: 1024,
    archived: true,
  ...
  {
    name: '5000-turns-archived-1-tool',
    turns: 5000,
    toolsPerTurn: 1,
    outputSize: 1024,
    archived: true,
  ...
  {
    name: '10000-turns-archived-1-tool',
    turns: 10000,
    toolsPerTurn: 1,
    outputSize: 1024,
    archived: true,
...
function itemStringBytes(items: Item[]): number {
  ...
  for (const item of items) {
    ...
    if (item.kind === 'tool')
      total += item.args.length + (item.output?.length ?? 0) + (item.error?.length ?? 0);
  }
  return total;
}
