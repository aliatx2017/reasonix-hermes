...
eq(
  sent.items[0].kind === 'user' && sent.items[0].text,
  'hello',
  'bubble carries the submitted text',
);
...
const failedState = reducer(sent, {
  type: 'send_failed',
  error: 'Send failed: bridge unavailable',
});
const failedBubble = failedState.items.find((it) => it.kind === 'user');
eq(
  failedBubble?.kind === 'user' && failedBubble.failed,
  true,
