...
function eq<T>(a: T, b: T, label: string) {
  if (a === b) {
    ...
    process.stdout.write(
      `  FAIL  ${label}: expected ${JSON.stringify(b).slice(0, 120)}, got ${JSON.stringify(a).slice(0, 120)}\n`,
    );
    failed += 1;
  }
}
...
function addTools(state: TestState, count: number, argsLen = 5000, outputLen = 10000): TestState {
  ...
  for (let i = 0; i < count; i++) {
    ...
    s = reducer(s, {
      ...
      e: {
        kind: 'tool_dispatch',
        tool: { id, name: 'bash', args: 'x'.repeat(argsLen), readOnly: false },
      },
    });
    s = reducer(s, {
      type: 'event',
      e: {
        kind: 'tool_result',
        tool: { id, name: 'bash', readOnly: false, output: 'y'.repeat(outputLen), durationMs: 100 },
      },
    });
  }
  return s;
...
// ── Test 1: Every completed tool is archived immediately ──
{
  ...
  ok(
    (tools[0].args?.length ?? 0) <= 205,
    `args truncated to ≤200 chars (got ${tools[0].args?.length})`,
  );
}
...
// ── Test 3: Undefined output doesn't crash ──
{
  ...
  s = reducer(s, {
    ...
    e: {
      kind: 'tool_dispatch',
      tool: { id: 'noop', name: 'glob', args: JSON.stringify({ pattern: '**/*' }), readOnly: true },
    },
  });
  s = reducer(s, {
    type: 'event',
    e: {
      kind: 'tool_result',
      tool: { id: 'noop', name: 'glob', readOnly: true, output: undefined, durationMs: 5 },
    },
  });
  const tools = toolItems(s);
...
// ── Test 4: Running (in-flight) tools keep full args for subject/UI ──
{
  ...
  s = reducer(s, {
    ...
    e: {
      kind: 'tool_dispatch',
      tool: { id: 'run1', name: 'bash', args: '{"command":"echo hello"}', readOnly: false },
    },
  });
  ...
  s = reducer(s, {
    ...
    e: {
      kind: 'tool_result',
      tool: { id: 'run1', name: 'bash', readOnly: false, output: 'hello world', durationMs: 50 },
    },
  });
...
// ── Test 5: Total string size reduction in a long session ──
{
  ...
  const totalStringBytes = tools.reduce(
    (sum, t) => sum + (t.args?.length ?? 0) + (t.output?.length ?? 0),
    0,
  );
  // Expected: each tool has ~200 chars args + 0 output = ~200 per tool
  const expectedMax = TOOL_COUNT * 205;
  ok(
    totalStringBytes <= expectedMax,
    `total string size ${totalStringBytes.toLocaleString()} ≤ ${expectedMax.toLocaleString()} (${((100 * totalStringBytes) / expectedMax).toFixed(0)}% of max)`,
  );
