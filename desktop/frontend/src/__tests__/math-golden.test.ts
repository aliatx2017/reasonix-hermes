...
eq(
  latexNormalizeForKatex('\\text{cost is $5}'),
  '\\text{cost is \\textdollar{}5}',
  'escapes $ in \\text',
);
eq(
  latexNormalizeForKatex('\\text{a & b % c_d ^ e ~ f}'),
  '\\text{a \\& b \\% c\\_d \\textasciicircum{} e \\textasciitilde{} f}',
  'escapes & % _ ^ ~ in \\text',
);
eq(
  latexNormalizeForKatex('\\text{already\\_escaped}'),
  '\\text{already\\_escaped}',
  'no double-escape',
);
...
eq(
  latexNormalizeForKatex('\\textbf{hello world}'),
  '\\textbf{hello world}',
  '\\textbf no special chars',
);
eq(latexNormalizeForKatex('\\tfrac{a}{b}'), '\\tfrac{a}{b}', 'nested braces in command');
eq(
  latexNormalizeForKatex('\\|x\\|'),
  '\\|x\\|',
  '\\| is left alone (readCommand handles \\|, not | branch)',
);
eq(
  latexNormalizeForKatex('\\\\|x|'),
  '\\\\\\vert x\\vert',
  '\\\\| line break + pipe: both | → \\vert',
);
...
eq(
  latexNormalizeForKatex('a = b \\tag{10}'),
  'a = b \\tag{10}',
  '\\tag without aligned passes through',
);
eq(
  latexNormalizeForKatex('\\begin{aligned} a &= b \\\\ \\end{aligned}'),
  '\\begin{aligned} a &= b \\\\ \\end{aligned}',
  'aligned without \\tag unchanged',
);
eq(
  latexNormalizeForKatex('\\begin{aligned} a &= b \\tag{10}\\\\ c &= d \\end{aligned}'),
  '\\begin{align} a &= b \\tag{10}\\\\ c &= d \\end{align}',
  'aligned with \\tag → align',
);
eq(
  latexNormalizeForKatex('\\begin{aligned} a &= b \\tag{10}\\\\ c &= d \\tag{11} \\end{aligned}'),
  '\\begin{align} a &= b \\tag{10}\\\\ c &= d \\tag{11} \\end{align}',
  'aligned with multiple \\tag → align',
);
eq(
  latexNormalizeForKatex('\\boxed{\\begin{aligned} a &= b \\tag{10}\\\\ c &= d \\end{aligned}}'),
  '\\boxed{\\begin{align} a &= b \\tag{10}\\\\ c &= d \\end{align}}',
  'boxed aligned with \\tag → boxed align',
);
eq(
  latexNormalizeForKatex('\\begin{gathered} a = b \\tag{10}\\\\ c = d \\end{gathered}'),
  '\\begin{gather} a = b \\tag{10}\\\\ c = d \\end{gather}',
  'gathered with \\tag → gather',
);
...
check(
  'uppercase $I$ → NOT math (Roman numeral / acronym)',
  () => isLikelyInlineMath('I') === false,
);
...
eq(
  normalizeMath('$\\slashed\\epsilon(0)$'),
  '$\\not{\\epsilon(0)}$',
...
eq(
  normalizeMath('solve $x^2 + y^2 = z^2$ please'),
  'solve $x^2 + y^2 = z^2$ please',
  '$x^2+y^2$ is math',
);
...
eq(
  normalizeMath('```\necho $PATH$\n```\n\nsolve $x^2$'),
  '```\necho $PATH$\n```\n\nsolve $x^2$',
  'fenced code protected while prose math renders',
);
...
const e2e: Array<[string, string]> = [
  ...
  [
    ...
    'boxed aligned with \\tag → align (no error)',
  ],
  [
    '\\[\\boxed{\\begin{aligned}\nx &= 1 \\\\\ny &= 2\n\\end{aligned}}\\]',
    'LLM-native boxed aligned',
  ],
];
...
const passthrough: Passthrough[] = [
  ...
  {
    src: 'line break \\\\[4pt] here',
    expected: 'line break \\\\[4pt] here',
    label: 'LaTeX line-break spacing',
  },
