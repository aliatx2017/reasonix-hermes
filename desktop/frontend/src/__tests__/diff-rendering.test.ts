...
const styles = readFileSync(resolve(testDir, '../styles.css'), 'utf8').replace(
  /\/\*[\s\S]*?\*\//g,
  '',
);
...

{
  ...
  eq(
    JSON.stringify(rows.map((r) => [r.type, r.oldLine ?? '', r.newLine ?? '', r.text])),
    JSON.stringify([
      ['ctx', 1, 1, 'one'],
      ['del', 2, '', 'two'],
      ['add', '', 2, 'TWO'],
...
for (const prefix of ['diff', 'inline-diff']) {
  eq(
    finalDeclaration(`.${prefix}__table`, 'min-width'),
    'max-content',
    `${prefix} rows share the longest scroll width`,
  );
  eq(
    finalDeclaration(`.${prefix}__table`, 'width'),
    '100%',
    `${prefix} table fills the visible viewport`,
  );
  eq(
    finalDeclaration(`.${prefix}__row`, 'width'),
    '100%',
    `${prefix} row background fills table width`,
  );
  eq(
    finalDeclaration(`.${prefix}__gutter`, 'position'),
    'sticky',
    `${prefix} gutter remains visible while horizontally scrolled`,
  );
  eq(
    finalDeclaration(`.${prefix}__gutter`, 'left'),
    '0',
    `${prefix} sticky gutter anchors at left`,
  );
}
  ...
  s = reducer(s, {
    ...
    e: {
      ...
      tool: {
        ...
        name: 'edit_file',
        readOnly: false,
        output: 'edited settings/settings.gd',
        durationMs: 12,
      },
    },
  });
...

{
  ...
  eq(
    tool?.summary,
    '2 edits · 5 replacements',
    'replace_all multi_edit uses applied replacement count',
  );
}
