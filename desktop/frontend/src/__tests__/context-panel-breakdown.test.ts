...
eq(
  oversized.promptTokens +
    oversized.completionTokens +
    oversized.reasoningTokens +
    oversized.otherTokens,
  61_000,
  'oversized provider breakdown is normalized to used context tokens',
);
eq(
  Math.round(oversized.otherPct * 10) / 10,
  6.1,
  'oversized provider breakdown does not fill the ring',
);
...
ok(
  /\$|USD|US\$/.test(usdLocalized) && usdLocalized.includes('0.1759'),
  'ISO USD cost renders with locale-aware currency formatting',
);
const cnyLocalized = formatMoneyLocalized(12.3, 'CNY', { locale: 'zh' });
ok(
  /¥|CNY|CN¥/.test(cnyLocalized) && cnyLocalized.includes('12.30'),
  'ISO CNY cost renders with locale-aware currency formatting',
);
eq(
  formatMoneyLocalized(0.1759, 'A$', { locale: 'en' }),
  'A$0.1759',
  'symbol currency remains symbol-based',
);
eq(
  formatMoneyLocalized(0, 'USD', { locale: 'en', empty: 'dash' }),
  '-',
  'localized money preserves dash empty state',
);
