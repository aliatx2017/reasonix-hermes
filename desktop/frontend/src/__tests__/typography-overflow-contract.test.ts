...
eq(
  finalDeclaration(':root', '--sans'),
  'var(--font-ui)',
  'legacy sans alias stays synced with UI font',
);
eq(
  finalDeclaration(':root[data-text-size="xxlarge"]', '--font-scale'),
  '1.32',
  'xxlarge has a real scale bump',
);
ok(
...
eq(
  finalDeclaration('.transcript--empty', 'overflow-y'),
  'auto',
  'empty transcript can scroll instead of clipping',
);
eq(
  finalDeclaration('.welcome', 'overflow'),
  'visible',
  'welcome empty state is not clipped by its own box',
);
ok(
...
ok(
  /@media\s*\(max-width:\s*900px\)[\s\S]*?\.settings-field\s*\{[\s\S]*?grid-template-columns\s*:\s*1fr/.test(
    styles,
  ),
  'settings fields collapse to one column at the mid-width breakpoint',
);
ok(
  /@media\s*\(max-width:\s*760px\)[\s\S]*?\.settings-modal\s*\{[\s\S]*?width\s*:\s*100vw[\s\S]*?height\s*:\s*100vh/.test(
    styles,
  ),
