...
const stylesSource = readFileSync(resolve(testDir, '../styles.css'), 'utf8').replace(
  /\/\*[\s\S]*?\*\//g,
  '',
);
...
ok(
  finalDeclaration(
    ...
    'max-width',
  )?.includes('--chrome-panel-control-size'),
  'themed AppChrome tab lists reserve a flowing new-tab button slot',
);

ok(
  finalDeclaration(':root[data-theme-style] .app-chrome--tabs .tabbar__tabs', 'flex') ===
    '0 1 auto',
  'themed AppChrome tab lists size to tab content before shrinking',
);

ok(
  finalDeclaration(':root[data-theme-style] .app-chrome--tabs .tabbar__tabs', 'width') ===
    'max-content',
...
ok(
  /workbenchChrome \? \(\s*<span className="app-chrome__spacer" aria-hidden="true" \/>/s.test(
    appChromeSource,
  ),
...
ok(/topicbar__chrome-btn/.test(appSource), 'workbench keeps chrome controls in the topic bar');
...
for (const selector of [
  ...
  const rightSpace =
    finalDeclaration(selector, 'padding-right') ?? finalDeclaration(selector, 'padding') ?? '';
  ok(
    rightSpace.includes('--chrome-toggle-size') &&
...
ok(
  /@media \(max-width: 820px\) \{[\s\S]*\.app--darwin \.layout--workbench-chrome-hidden \.topicbar\s*\{[\s\S]*padding-left:\s*96px;/.test(
    stylesSource,
  ) &&
    /@media \(max-width: 820px\) \{[\s\S]*\.app--darwin \.layout--workbench-chrome-hidden\.layout--workspace-maximized \.workbench-dock__tools\s*\{[\s\S]*padding-left:\s*96px;/.test(
      stylesSource,
    ),
