...
function eq<T>(a: T, b: T, label: string) {
  if (a === b) {
    ...
    process.stdout.write(
      `  FAIL  ${label}: expected ${JSON.stringify(b).slice(0, 200)}, got ${JSON.stringify(a).slice(0, 200)}\n`,
    );
    failed += 1;
  }
}
...
// AssistantMessage's React.memo to skip re-render.
{
  ...
  function computeShown(item: SimAssistantItem, live: SimLiveStream | undefined) {
  ...
  eq(
    result1.reasoningComplete,
    result2.reasoningComplete,
    'shown.reasoningComplete stable when live unchanged',
  );
...
// also prevents re-highlighting for repeated calls with the same code.
{
  ...
  ok(
    time2 <= time1 || time2 < 1,
    `cached call (${time2.toFixed(2)}ms) not slower than first call (${time1.toFixed(2)}ms)`,
  );
