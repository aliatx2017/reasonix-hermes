    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {nodes.map((n) => (
        ...
        >
          ...
          <span style={{
            fontSize: 10, padding: "1px 5px", borderRadius: 3,
            background: n.status === "completed" ? "var(--color-green-subtle)" : n.status === "running" ? "var(--color-accent-subtle)" : "var(--color-surface)",
            color: n.status === "completed" ? "var(--color-green)" : n.status === "running" ? "var(--color-accent)" : "var(--color-text-muted)",
            marginLeft: "auto",
          }}>{n.status}</span>
