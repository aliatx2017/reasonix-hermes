...
export function MarketplacePanel(_props: MarketplacePanelProps) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setStatusMsg(`Synced ${result.added} new skills from LobeHub (${result.fetched} total fetched)`);
      // Reload skill list
  ...
  if (loading) {
    return (
      <div className="hermes-panel" style={{ padding: 12, fontSize: 12, color: "var(--color-text-muted)" }}>
        Loading marketplace...
      </div>
    );
  }
  ...
  return (
    <div className="hermes-panel" style={{ padding: 12, fontSize: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
        ...
        >
          <RefreshCw size={10} className={syncing ? "spin" : ""} /> {syncing ? "Syncing…" : "Sync from LobeHub"}
        </button>
      </div>
          ...
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            <span
              ...
              style={{
                ...
                background: activeTag === "" && !search ? "var(--color-accent)" : "var(--color-surface-raised)",
            ...
            {allTags.map((tag) => (
              <span
                ...
                style={{
                  padding: "1px 6px", borderRadius: 4, fontSize: 10, cursor: "pointer",
                  background: activeTag === tag ? "var(--color-accent)" : "var(--color-surface-raised)",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 300, overflowY: "auto" }}>
        {filtered.length === 0 ? (
          <div style={{ color: "var(--color-text-muted)", fontStyle: "italic", padding: 16, textAlign: "center" }}>
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  background: installed.has(skill.name) ? "var(--color-surface-raised)" : "transparent",
                  color: installed.has(skill.name) ? "var(--color-success, #4caf50)" : "var(--color-accent)",
                  cursor: "pointer",
                  fontSize: 10,
                  fontWeight: 500,
                  flexShrink: 0,
      ...
      {/* Status message */}
      {statusMsg && (
        <div style={{ marginTop: 8, color: "var(--color-success, #4caf50)", fontSize: 10, fontStyle: "italic" }}>
          {statusMsg}
        </div>
      )}
