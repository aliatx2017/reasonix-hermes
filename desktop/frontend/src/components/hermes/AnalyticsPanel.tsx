...
export function AnalyticsPanel() {
  ...
  const cacheHitRate =
    totalCacheHit + totalCacheMiss > 0 ? totalCacheHit / (totalCacheHit + totalCacheMiss) : 0;
  ...
  return (
    <section className="settings-section">
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div
          style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 80, padding: '0 4px' }}
        >
          {timeline.map((p) => {
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div
          style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 40, padding: '0 4px' }}
        >
          {timeline.map((p) => {
            ...
            return (
              <div
                key={p.turn}
                style={{ flex: 1, height: 40, display: 'flex', flexDirection: 'column' }}
                title={`Turn ${p.turn}: ${fmtNum(p.cacheHitTokens)} hit / ${fmtNum(p.cacheMissTokens)} miss`}
              >
                <div
                  style={{ flex: 1, background: 'var(--color-green)', borderRadius: '2px 2px 0 0' }}
                />
                <div
                  style={{
                    height: `${missPct}%`,
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span
              style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--color-green)' }}
            />{' '}
            Cache hit
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {topTools.map(([name, count], i) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    style={{ minWidth: 32, textAlign: 'right', color: 'var(--color-text-muted)' }}
                  >
                    {count}
                  </span>
                </div>
      ...
      <div>
        ...
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: `repeat(${Math.min(timeline.length, 8)}, 1fr)`,
            gap: 4,
            fontSize: 10,
            overflowX: 'auto',
          }}
