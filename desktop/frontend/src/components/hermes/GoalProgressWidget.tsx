...
export function GoalProgressWidget({ goal, compact }: GoalProgressWidgetProps) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        ...
        <span
          style={{
            ...
            padding: "1px 6px",
            borderRadius: 4,
            background: goal.status === "running" ? "var(--color-accent-subtle)" : "var(--color-surface)",
            border: "1px solid var(--color-border)",
            display: "inline-flex",
            alignItems: "center",
            gap: 4,
      ...
      <div style={{ display: "flex", gap: 16, fontSize: 12, color: "var(--color-text-muted)", marginTop: 6 }}>
        <span>{goal.turns} turns</span>
        {goal.blocks > 0 && <span>{goal.blocks} blocks</span>}
      </div>
      {/* Progress bar */}
      <div style={{ marginTop: 6, height: 3, borderRadius: 2, background: "var(--color-border)", overflow: "hidden" }}>
        <div
          style={{
            height: "100%",
            borderRadius: 2,
            background:
              goal.status === "complete" ? "var(--color-green)"
              : goal.status === "blocked" ? "var(--color-yellow)"
              : "var(--color-accent)",
