  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: 'var(--color-text-muted)', margin: '0 0 12px' }}>
        Browse and sync skills from the <strong>LobeHub</strong> community marketplace (360,000+
        community skills). Use the sync button below to pull the latest skills.
      </p>
      {lastSync && (
        <div
          style={{
        ...
        >
          Last synced: {new Date(lastSync.lastSync).toLocaleDateString()} — {lastSync.fetched}{' '}
          skills fetched, {lastSync.added} new.
        </div>
      )}
      <MarketplacePanel />
    </div>
...
function MCPTab() {
  ...
  return (
    <div style={{ padding: '12px 0' }}>
      <p style={{ fontSize: 12, color: 'var(--color-text-muted)', margin: '0 0 12px' }}>
        MCP servers extend Reasonix with external tools, data sources, and integrations. Manage
        installed servers in <strong>Settings → MCP</strong>.
      </p>
      {loading ? (
        <div
          style={{
