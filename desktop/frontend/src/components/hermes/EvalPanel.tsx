...
export function EvalPanel() {
  ...
  useEffect(() => {
    app.ListSessions().then(setSessions).catch(() => setSessions([]));
    // Refresh every 15s in case new sessions are created.
    const iv = setInterval(() => {
      app.ListSessions().then(setSessions).catch(() => {});
    }, 15000);
  ...
  return (
    <section className="settings-section">
      ...
      <p className="settings-section__desc">
        Compare two saved agent sessions — tool usage, token efficiency, turn similarity.
        Use <code>reasonix eval compare &lt;a&gt; &lt;b&gt;</code> from the CLI.
      </p>
      ...
      {error && (
        <div style={{ marginTop: 8, padding: "8px 12px", borderRadius: 6, background: "var(--color-error-subtle)", color: "var(--color-error)", fontSize: 13 }}>
          {error}
        </div>
      )}
      ...
      {result && (
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{
            ...
            <StatCard label="Turns" a={`${result.turnsA}`} b={`${result.turnsB}`}
              delta={result.turnsB - result.turnsA} />
            <StatCard label="Tokens In" a={fmtNum(result.tokensInA)} b={fmtNum(result.tokensInB)}
              delta={result.tokensInB - result.tokensInA} />
            <StatCard label="Tokens Out" a={fmtNum(result.tokensOutA)} b={fmtNum(result.tokensOutB)}
              delta={result.tokensOutB - result.tokensOutA} />
            <StatCard label="Cost" a={`${result.currency}${result.costA.toFixed(4)}`}
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{
              ...
              <div style={{
                width: `${Math.round(result.similarity * 100)}%`, height: "100%",
          ...
          {result.toolDiffs.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              <h4 style={{ fontSize: 13, fontWeight: 600, margin: "0 0 6px", color: "var(--color-text-muted)" }}>
                Tool Usage
              </h4>
              <table style={{
                ...
                <tbody>
                  {result.toolDiffs.filter((d) => d.countA > 0 || d.countB > 0).map((d) => (
                    <tr key={d.name} style={{ borderBottom: "1px solid var(--color-border)" }}>
                      ...
                      <td style={{ ...td, textAlign: "right", color: d.delta > 0 ? "var(--color-green)" : d.delta < 0 ? "var(--color-red)" : "var(--color-text-muted)" }}>
          ...
          {result.turnDiffs.length > 0 && (
            <div>
              <h4 style={{ fontSize: 13, fontWeight: 600, margin: "0 0 6px", color: "var(--color-text-muted)" }}>
                Turn Comparison ({result.turnDiffs.filter((t) => t.match).length}/{result.turnDiffs.length} matched)
              </h4>
              <div style={{
                display: "flex", flexWrap: "wrap", gap: 4, maxHeight: 160, overflowY: "auto",
                padding: 4,
              }}>
      ...
      {!result && !loading && !error && (
        <div style={{
          marginTop: 16, padding: "24px", borderRadius: 8,
...
function StatCard({ label, a, b, delta }: { label: string; a: string; b: string; delta: number }) {
  const deltaColor = delta > 0 ? "var(--color-green)" : delta < 0 ? "var(--color-red)" : "var(--color-text-muted)";
  const sign = delta > 0 ? "+" : "";
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
        ...
        {delta !== 0 && (
          <span style={{ fontSize: 11, color: deltaColor, fontWeight: 600 }}>
            {sign}{typeof delta === "number" && !label.startsWith("Cost") ? delta : delta.toFixed(4)}
          </span>
        )}
      </div>
    </div>
...
const th: React.CSSProperties = {
  padding: "6px 10px", fontWeight: 600, color: "var(--color-text-muted)",
  fontSize: 11, textAlign: "left", borderBottom: "1px solid var(--color-border)",
};
const td: React.CSSProperties = {
  padding: "5px 10px", fontSize: 12, color: "var(--color-text)",
};
