...
export function ProfilePicker({
  ...
  return (
    <div className="profile-picker" style={{
      ...
      <select
        ...
        title={active ? `${active[0]}: ${active[1].description || ""} (model: ${active[1].model || "default"})` : "Select harness profile"}
        style={{
          ...
          color: "var(--color-text)", maxWidth: 120, cursor: "pointer",
        }}
      >
        <option value="__none__">profile</option>
        <option value="__clear__" disabled={!activeProfile}>— clear —</option>
        {entries.map(([name]) => (
