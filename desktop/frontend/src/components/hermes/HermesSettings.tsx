...
export function HermesSettings({
  s,
  onHotbarChange: _onHotbar,
  onProfileSelect: _onProfile,
  cache,
  bot,
  goal,
  cost,
  ...
  return (
    <div className="hermes-settings" style={{ padding: '16px 0' }}>
      ...
      <section className="settings-section">
        ...
        >
          ...
          {compress && (compress.bytesSaved > 0 || compress.auxTokens > 0) && (
            ...
            >
              {compress.bytesSaved > 0 && (
                <span title="Compressor bytes saved">
                  sqz {formatCompactBytes(compress.bytesSaved)}
                </span>
              )}
              {compress.auxTokens > 0 && (
                <span title="Tokens routed to cheaper providers">
                  aux ↓{compress.auxTokens.toLocaleString()}
                </span>
              )}
            </div>
          )}
      ...
      <hr
        style={{ margin: '24px 0', border: 'none', borderTop: '1px solid var(--color-border)' }}
      />
      ...
      <section className="settings-section">
        ...
        >
          Per-turn file snapshots captured before the agent writes or edits. Click a turn to see
          pre-edit contents.
        </p>
            ...
            return (
              ...
              >
                ...
                <span
                  style={{
                    fontSize: 13,
                    color: isDefault ? 'var(--color-text-muted)' : 'var(--color-text)',
                  }}
                >
                  {label}
...
tool_approve_mode = "yolo"`}
      ...
      <section className="settings-section">
        ...
        <p className="settings-section__desc">
          Browse, sync, and install skills from the community. Four categories: LobeHub (360k+),
          Market (agentskills.io), MCP servers, and Custom project skills.
        </p>
        <SkillStorePanel />
      </section>
      ...
      <section className="settings-section">
        ...
        <p className="settings-section__desc">
          Sub-agent tasks spawned by the model via the <code>task</code> tool or subagent skills.
          Shows active and completed sub-agents for this session.
        </p>
        <SubagentTreePanel />
      </section>
      ...
      <section className="settings-section">
        ...
        <p className="settings-section__desc">
          Reads <code>.reasonix/constitution.json</code> from the project root. Shows principles,
          hard constraints, and code-level rules with severity.
        </p>
      ...
      <section className="settings-section">
        ...
        <p className="settings-section__desc">
          ...
          (v1.6.0). Adds Discord bot, MCP bridge server, Hindsight memory, curated skills hub,
          native hook runner, portable mode, and desktop hotbar + harness profiles.
        </p>
