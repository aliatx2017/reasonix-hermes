  ...
  useEffect(() => {
    try {
      ...
      if (w.runtime?.EventsOn) {
        ...
        app.CompactionHistory().then(setEvents).catch(() => {});
        return () => { try { unsub(); } catch { /* ignore */ } };
      }
    } catch { /* fall through */ }

    app.CompactionHistory().then(setEvents).catch(() => {});
    const id = setInterval(() => app.CompactionHistory().then(setEvents).catch(() => {}), 5000);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ display: "flex", gap: 12, fontSize: 11, color: "var(--color-text-muted)", marginBottom: 4 }}>
        <span><History size={11} style={{ marginRight: 3, verticalAlign: -1 }} />{events.length} passes</span>
        <span><FolderOpen size={11} style={{ marginRight: 3, verticalAlign: -1 }} />{totalFolded} msgs folded</span>
      </div>
