...
export function CheckpointFileList({ checkpoints }: { checkpoints: CheckpointMeta[] }) {
  ...
  const loadDiff = async (turn: number, relPath: string) => {
    ...
    setDiffLoading((s) => { const ns = new Set(s); ns.delete(key); return ns; });
  };
                  ...
                  return (
                    <div key={s.path} style={{ marginTop: 4 }}>
                      <div style={{
                        ...
                        {s.content && (
                          <button
                            onClick={(e) => { e.stopPropagation(); loadDiff(cp.turn, s.path); }}
                            style={{
                              marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 2,
                              padding: "1px 5px", fontSize: 10, border: "1px solid var(--color-border)",
                              borderRadius: 3, background: "var(--bg)", color: "var(--fg)", cursor: "pointer",
                            }}
                          >
                            <GitCompare size={9} />
                      ...
                      {diff && !diff.same && (
                        <div style={{
                          marginTop: 2, padding: "2px 4px", background: "var(--code-bg)",
                          borderRadius: 4, maxHeight: 200, overflow: "auto", fontSize: 11,
                        }}>
                          <DiffView original={diff.oldText} modified={diff.newText} language="text" />
                        </div>
                      )}
                      {diff?.same && (
                        <div style={{
