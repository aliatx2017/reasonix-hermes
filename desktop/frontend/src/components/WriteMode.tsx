...
export function WriteMode() {
  ...
  const [openFiles, setOpenFiles] = useState<
    { relPath: string; name: string; content: string; dirty: boolean }[]
  >([]);
  ...
  const saveFile = useCallback(async () => {
    ...
    try {
      ...
      setOpenFiles((prev) =>
        prev.map((f, i) => (i === activeFileIdx ? { ...f, dirty: false } : f)),
      );
    } catch {
      /* ignore */
    }
  ...
  const handleContentChange = useCallback(
    (value: string) => {
      setOpenFiles((prev) =>
        prev.map((f, i) => (i === activeFileIdx ? { ...f, content: value, dirty: true } : f)),
      );
      ...
      autoSaveTimer.current = setTimeout(() => {
        setOpenFiles((prev) => {
          ...
          if (file?.relPath) {
            app
              .SaveMarkdownFile(file.relPath, value)
              .then(() => {
                setOpenFiles((p) =>
  ...
  const createFile = useCallback(async () => {
    ...
    try {
      const relPath = await app.CreateMarkdownFile(
        newFileName.trim(),
        '# ' + newFileName.trim() + '\n\n',
      );
      if (relPath) {
        ...
        {showNewFile && (
          ...
          >
            <input
              ...
              autoFocus
            />
            <button onClick={createFile} style={iconBtnStyle}>
              <Plus size={10} />
            </button>
            <button onClick={() => setShowNewFile(false)} style={iconBtnStyle}>
              <X size={10} />
            </button>
          </div>
        )}
          ...
          {files.map((f) => (
            <button
              ...
              style={{
                ...
                padding: '5px 10px',
                border: 'none',
                background: selectedFile === f.relPath ? 'var(--sidebar-active)' : 'none',
                cursor: 'pointer',
                fontSize: 11,
                color: selectedFile === f.relPath ? 'var(--accent)' : 'var(--fg)',
                textAlign: 'left',
              }}
            >
              <FileText size={11} />
            ...
            {openFiles.map((f, i) => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  borderBottom:
                    i === activeFileIdx ? '2px solid var(--accent)' : '2px solid transparent',
                  background: i === activeFileIdx ? 'var(--bg)' : 'none',
                  cursor: 'pointer',
                  fontSize: 11,
                  color: i === activeFileIdx ? 'var(--fg)' : 'var(--color-text-muted)',
                  whiteSpace: 'nowrap',
                }}
              >
                <FileText size={10} />
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <button
                onClick={handleFIM}
                disabled={fimBusy}
                title="FIM completion (Ctrl+Space)"
                style={{
                  ...iconBtnStyle,
                  opacity: fimBusy ? 0.5 : 1,
                }}
              >
                <Wand2 size={12} />
              ...
              <button
                ...
                style={{
                  ...iconBtnStyle,
                  fontWeight: viewMode !== 'edit' ? 700 : 400,
              ...
              >
                ...
                <span style={{ fontSize: 10 }}>
                  {viewMode === 'edit' ? 'Preview' : viewMode === 'split' ? 'Edit' : 'Edit'}
                </span>
              </button>
              <button
                onClick={saveFile}
