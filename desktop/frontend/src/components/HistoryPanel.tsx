...
import { type Item, historyMessagesToItems } from '../lib/useController';
...
>>>>>>> upstream/main-v2
  ...
  const dateCounts = useMemo(() => {
    const counts: Record<HistoryDateFilter, number> = {
      all: sessions.length,
      today: 0,
      yesterday: 0,
      older: 0,
  ...
  const filteredSessions = useMemo(() => {
    ...
    return sessions.filter((s) => {
      ...
      if (dateFilter !== 'all' && dateBucket(sessionTimeForGrouping(s, isTrash)) !== dateFilter)
        return false;
      if (!q) return true;
      return [s.title, s.preview, s.path, s.topicTitle, s.workspaceRoot].some((part) =>
  ...
  const previewItems = useMemo(
    () => previewMessagesToItems(preview?.messages ?? []),
    [preview?.messages],
  );
  ...
  const openTrashBlankMenu = (event: ReactMouseEvent<HTMLDivElement>) => {
    ...
    if (target?.closest('.hist-item,.history-search,.history-preview,button,input,textarea,select'))
      return;
    event.preventDefault();
    setMenuConfirmTarget(null);
    setMenuSession(null);
    ...
    ? isTrash
      ? [
          ...
          { type: 'separator', key: 'trash-session-separator' },
          {
            ...
            onSelect: () => {
              if (
                menuConfirmTarget?.kind === 'purge' &&
                menuConfirmTarget.path === menuSession.path
              ) {
  ...
  const actionConfirmDelete =
    selectedSession &&
    menuConfirmTarget?.kind === 'delete' &&
    menuConfirmTarget.path === selectedSession.path;
  ...
  return (
    <div
      className="management-modal-backdrop history-modal-backdrop"
      data-state={status}
      onClick={(e) => {
    ...
    >
      ...
      >
        <header className="management-modal__head history-modal__head">
          <div>
            <div className="management-modal__title history-modal__title">
              {tr(isTrash ? 'history.trashTitle' : 'history.title')}
            </div>
            {!isTrash && running && (
...
>>>>>>> upstream/main-v2
        ...
        <div className="history-manager" onContextMenu={openTrashBlankMenu}>
            ...
            {!isTrash && (
              <HistoryFilterSelect
                ...
                options={[
                  ...
                  {
                    id: 'current',
                    label: tr('history.filterCurrent'),
                    count: statusCounts.current,
                  },
                      ...
                      return (
                        ...
                        >
                          {editing === s.path ? (
                            ...
                            >
                              <div className="hist-item__preview">
                                {sessionDisplayTitle(s, tr('history.emptySession'))}
                              </div>
                              <div className="hist-item__meta">
                                {!isTrash && s.current && (
                                  <span className="hist-item__badge hist-item__badge--current">
                                    {tr('history.current')}
                                  </span>
                                )}
                                {!isTrash && !s.current && s.open && (
                                  <span className="hist-item__badge hist-item__badge--open">
                                    {tr('history.open')}
                                  </span>
                                )}
                                {isTrash && (
                                  <span className="hist-item__badge hist-item__badge--deleted">
                                    {tr('history.deleted')}
                                  </span>
                                )}
                                {sessionLocation(s, tr) && (
                                  <span className="hist-item__scope">{sessionLocation(s, tr)}</span>
                                )}
                                <span className="hist-item__metaspacer" />
                                ...
                                <span className="hist-item__stat">
                                  {timeLabel(
                                    isTrash
                                      ? s.deletedAt || sessionActivityTime(s)
                                      : sessionActivityTime(s),
                                  )}
                                </span>
                                {!isTrash && running && (
                                  <>
          ...
          <ContextMenu
            ...
            minWidth={220}
            ariaLabel={
              isTrash ? tr('history.trashSessionActions') : tr('history.historySessionActions')
            }
...
function HistoryFilterSelect({
  ...
  const visibleOptions = options.filter(
    (option) => option.id === 'all' || option.id === value || option.count > 0,
  );
  return (
