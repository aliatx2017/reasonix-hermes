      ...
      {archives.length === 0 ? (
        ...
        <div className="mem-facts mem-facts--archive">
          {archives.map((f) => {
            ...
            return (
              <article
                className="mem-fact mem-fact--archived"
                ...
                >
                  ...
                  <span className="mem-fact__main">
                    ...
                    <span className="mem-fact__meta">
                      {f.type && (
                        <span className="mem-fact__type" data-mem-type={f.type}>
                          {memoryTypeLabel(f.type, t)}
                        </span>
                      )}
...
export function MemoryPanel({
  ...
  const filteredDocs = useMemo(() => {
    ...
    return view.docs.filter(
      (d) => d.body.toLowerCase().includes(q) || d.path.toLowerCase().includes(q),
    );
  }, [view, filter]);
      ...
      {!view?.available ? (
        ...
        <div className="drawer__body">
          ...
          <section className="mem-section">
                        ...
                        <div className="mem-fact__links" aria-label={t('memory.links')}>
                          {links.map((link) =>
                            link.exists ? (
                              ...
                              <Tooltip
                                key={link.name}
                                label={t('memory.deadLink', { name: link.name })}
                              >
                                <span className="mem-link-chip mem-link-chip--dead">
                                  [[{link.name}]]
                                </span>
                              </Tooltip>
                            ),
            ...
            {(view.storeDir || view.storeGlobalDir) && (
              <div className="mem-hint">
                {t('memory.storedUnder', {
                  dir: [view.storeDir, view.storeGlobalDir].filter(Boolean).join(' + '),
                })}
              </div>
            )}
          </section>
          ...
          <section className="mem-section">
            ...
            <div className="mem-hint">{scopes.find((s) => s.scope === activeScope)?.path}</div>
          </section>
          ...
          <section className="mem-section">
            ...
            {filteredDocs.length === 0 && (
              <div className="mem-empty">
                {filter ? t('memory.noFilterMatch') : t('memory.noDocs')}
              </div>
            )}
            {filteredDocs.map((d) => {
              ...
              return (
                <div className="mem-doc" data-doc-scope={d.scope || 'other'} key={d.path}>
                  <div className="mem-doc__head">
                    ...
                    <span className={`mem-doc__tag badge--${d.scope}`}>
                      {memoryScopeLabel(d.scope, t)}
                    </span>
                    {!editing && (
                      <button className="btn btn--small" onClick={() => startEdit(d.path, d.body)}>
                        {t('common.edit')}
                      </button>
                    )}
                  </div>
          ...
          <section className="mem-section">
            ...
            {filteredFacts.length === 0 ? (
              <div className="mem-empty">
                {filter ? t('memory.noFilterMatch') : t('memory.noFacts')}
              </div>
            ) : (
              filteredFacts.map((f) => (
            ...
            {(view.storeDir || view.storeGlobalDir) && (
              <div
                className="mem-hint"
                title={[view.storeDir, view.storeGlobalDir].filter(Boolean).join(' + ')}
              >
                {t('memory.storedUnder', {
                  dir: [view.storeDir, view.storeGlobalDir].filter(Boolean).join(' + '),
                })}
...
export function MemorySettingsPage() {
  ...
  useEffect(() => {
    if (
      ...
      suggestions ||
      suggestionBusy ||
      autoSuggestionsRequested.current
    )
      return;
  ...
  const filteredArchives = useMemo(
    () =>
      archives.filter((f) => {
        ...
        if (!normalizedQuery) return true;
        return (
          memoryMatches(f, normalizedQuery, 'all') ||
          [f.path, f.archivedAt].join(' ').toLowerCase().includes(normalizedQuery)
        );
      }),
  ...
  const acceptMemorySuggestion = useCallback(
  ...
  const acceptSkillSuggestion = useCallback(
  ...
  return (
    <>
      ...
      <div className="memory-tabs-row" role="tablist" aria-label={t('settings.tab.memory')}>
        ...
        <button
          className={
            'memory-suggestion-tab' +
            (tab === 'suggestions' ? ' memory-suggestion-tab--active' : '')
          }
          role="tab"
        ...
        >
          ...
          {suggestionTotal(suggestions) > 0 && (
            <span className="settings-subtab__count">{suggestionTotal(suggestions)}</span>
          )}
        </button>
      </div>
      {tabs.length > 1 && (
          ...
          {hasSavedFilters && (
            <div className="mem-toolbar">
              ...
              <div className="mem-filter" role="tablist" aria-label={t('memory.typeFilter')}>
                <button
                  className={
                    'mem-filter__item' + (typeFilter === 'all' ? ' mem-filter__item--on' : '')
                  }
                  onClick={() => setTypeFilter('all')}
                  type="button"
                >
                ...
                {factTypes.map((type) => (
                  <button
                    className={
                      'mem-filter__item' + (typeFilter === type ? ' mem-filter__item--on' : '')
                    }
                    onClick={() => setTypeFilter(type)}
                    type="button"
                    key={type}
        ...
        <section className="mem-section">
          ...
          <div className="mem-suggestion-settings">
            ...
            <Tooltip
              label={
                autoSuggestions
                  ? t('memory.disableAutoSuggestions')
                  : t('memory.enableAutoSuggestions')
              }
            >
            ...
            <div className="mem-suggestions">
              {suggestions.generatedAt && (
                <div className="mem-suggestions__stamp">
                  {t('memory.suggestionsGenerated', {
                    time: suggestionStamp(suggestions.generatedAt),
                  })}
                </div>
              )}
                ...
                <div className="mem-suggestion-group">
                  ...
                  <div className="mem-facts">
                    {suggestions.memories.map((candidate) => {
                      ...
                      return (
                        <article
                          className="mem-fact mem-suggestion"
                          data-mem-type={candidate.type || 'other'}
                          key={candidate.id}
                        >
                          <button
                            className="mem-fact__summary"
                          ...
                          >
                            ...
                            <span className="mem-fact__main">
                              <span className="mem-fact__title">
                                {candidate.title || candidate.name}
                              </span>
                              <span className="mem-fact__meta">
                                <span className="mem-fact__type" data-mem-type={candidate.type}>
                                  {memoryTypeLabel(candidate.type, t)}
                                </span>
                          ...
                          {open && (
                            <div className="mem-fact__detail">
                              ...
                              {candidate.reason && (
                                <div className="mem-suggestion__reason">{candidate.reason}</div>
                              )}
                              ...
                              <div className="mem-fact__actions">
                                <span className="mem-hint mem-hint--inline">
                                  {t('memory.confirmBeforeApply')}
                                </span>
                                {accepted ? (
                                  <span className="mem-suggestion__accepted">
                                    <Check size={13} />
                                    {t('memory.savedSuggestion')}
                                  </span>
                                ) : (
                                  <button
                                    className="btn btn--primary btn--small"
                ...
                <div className="mem-suggestion-group">
                  ...
                  <div className="mem-facts">
                    {suggestions.skills.map((candidate) => {
                      ...
                      return (
                        <article
                          className="mem-fact mem-suggestion mem-suggestion--skill"
                          data-mem-type="reference"
                          key={candidate.id}
                        >
                          <button
                            className="mem-fact__summary"
                          ...
                          >
                            ...
                            <span className="mem-fact__main">
                              ...
                              <span className="mem-fact__meta">
                                ...
                                <span className="mem-fact__slug">
                                  {memoryScopeLabel(candidate.scope, t)}
                                </span>
                              </span>
                              <span className="mem-fact__desc">{candidate.description}</span>
                            </span>
                          ...
                          {open && (
                            <div className="mem-fact__detail">
                              <pre className="mem-suggestion__body mem-suggestion__body--code">
                                {candidate.body}
                              </pre>
                              {candidate.reason && (
                                <div className="mem-suggestion__reason">{candidate.reason}</div>
                              ...
                              <div className="mem-fact__actions">
                                ...
                                {accepted ? (
                                  <span className="mem-suggestion__accepted">
                                    <Check size={13} />
                                    {t('memory.createdSkillSuggestion')}
                                  </span>
                                ) : (
                                  <button
                                    className="btn btn--primary btn--small"
