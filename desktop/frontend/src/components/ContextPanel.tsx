...
export function contextCostDisplay({
  ...
  if (info?.sessionCost && info.sessionCost > 0) {
    return {
      ...
      currency: info.sessionCurrency || sessionCurrency || usage?.currency,
    };
  }
  if (sessionCost && sessionCost > 0) {
    return {
      amount: sessionCost,
      currency: sessionCurrency || info?.sessionCurrency || usage?.currency,
    };
  }
  if (usage?.cost && usage.cost > 0) {
    return {
      amount: usage.cost,
      currency: usage.currency || sessionCurrency || info?.sessionCurrency,
    };
  }
...
export function contextBreakdown(
  ...
  let reasoning = Math.min(
    nonNegativeTokenCount(reasoningTokens),
    nonNegativeTokenCount(completionTokens),
  );
  ...
  const reasoningPct = hasWindow
    ? Math.min(100, ((prompt + completion + reasoning) / window) * 100)
    : 0;
...
export function ContextPanel({
  ...
  const windowTokens =
    context?.window && context.window > 0 ? context.window : (info?.windowTokens ?? 0);
  const promptTokens = hasPanelUsage ? (info?.promptTokens ?? 0) : (usage?.promptTokens ?? 0);
  const completionTokens = hasPanelUsage
    ? (info?.completionTokens ?? 0)
    : (usage?.completionTokens ?? 0);
  const totalTokens =
    info?.totalTokens && info.totalTokens > 0
      ? info.totalTokens
  ...
  const reasoningTokens = hasPanelUsage
    ? (info?.reasoningTokens ?? 0)
    : (usage?.reasoningTokens ?? 0);
  const cacheHitTokens = hasPanelUsage ? (info?.cacheHitTokens ?? 0) : (usage?.cacheHitTokens ?? 0);
  const cacheMissTokens = hasPanelUsage
  ...
  const usagePct =
    windowTokens > 0 ? Math.min(100, Math.round((usedTokens / windowTokens) * 100)) : 0;
  ...
  const derivedElapsed =
    eventTimes.length > 1 ? Math.max(...eventTimes) - Math.min(...eventTimes) : 0;
  ...
  const requestCount =
    info?.requestCount && info.requestCount > 0 ? info.requestCount : derivedRequestCount;
  const readRows = readFiles.map((f, i) => ({
    ...
    time: fmtTime(f.time),
    detail: f.limit
      ? `${f.offset ?? 0}-${(f.offset ?? 0) + f.limit}${f.truncated ? ' truncated' : ''}`
      : '',
  }));
      ...
      <div className="context-panel__body">
        <section className="context-panel__overview">
          <section className="context-panel__usage">
            ...
            <div className="context-panel__breakdown">
              <TokenLegend
                ...
                value={breakdown.promptTokens}
                color="prompt"
              />
              <TokenLegend
                label={t('context.completion')}
                value={breakdown.completionTokens}
                color="completion"
              />
              <TokenLegend
                label={t('context.reasoning')}
          ...
          <section className="context-panel__section">
            ...
            <div className="context-panel__stats">
              <MetricCard
                label={t('context.cacheHit')}
                value={cachePct > 0 ? `${cachePct}%` : '-'}
                tone="accent"
              />
              <MetricCard
          ...
          <section className="context-panel__section context-panel__section--status">
            ...
            <div className="context-panel__stats">
              <MetricCard
                ...
                value={t(health.shortKey, health.vars)}
                tone={health.tone}
              />
              <MetricCard
                label={t('context.compaction')}
                value={compactPct > 0 ? `${compactPct}%` : '-'}
              />
            </div>
          </section>
          <PreviewSection
...
function MetricCard({
  ...
  value,
  tone,
}: { label: string; value: string; tone?: 'accent' | 'good' | 'notice' | 'warn' }) {
...
function FileTable({
  ...
  return (
    <div
      className={`context-panel__file-list${compact ? ' context-panel__file-list--compact' : ''}`}
    >
      {rows.map((row) => {
        const content = (
          <>
