...
import { WORKSPACE_REF_DRAG_TYPE, formatWorkspaceReference } from '../lib/workspaceDrag';
...
const WORKSPACE_DUAL_PANEL_TARGET_WIDTH =
  WORKSPACE_TREE_DEFAULT_WIDTH + WORKSPACE_PREVIEW_TARGET_WIDTH;
...
export function WorkspacePanel({
  ...
  const selectFile = useCallback(
    (path: string) => {
      ...
      setFilter('');
      setOpenTabs((tabs) =>
        [...tabs.filter((tab) => tab !== path), path].slice(-WORKSPACE_MAX_PREVIEW_TABS),
      );
  ...
  useEffect(() => {
    ...
    const paths = Array.from(
      new Set(fileListRequest.paths.map((path) => path.trim()).filter(Boolean)),
    );
  ...
  const closeTab = (path: string) => {
    if (
      lastRevealRequestIdRef.current === revealPathRequest?.id &&
      revealPathRequest.path === path
    ) {
      dismissedRevealRequestIdRef.current = revealPathRequest.id;
    }
    if (
      lastChangeRevealRequestIdRef.current === changeRevealRequest?.id &&
      changeRevealRequest.path === path
    ) {
  ...
  const effectiveTreeWidth = useMemo(
    () => clampWorkspaceTreeWidth(treeWidth, panelWidth),
  ...
  const compactTreeRail =
    ...
    panelWidth < WORKSPACE_DUAL_PANEL_MIN_WIDTH;
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      const row = Array.from(
        treeRef.current?.querySelectorAll<HTMLElement>('[data-workspace-path]') ?? [],
  ...
  const panelStyle = useMemo(
    () =>
      ({
        ...
        '--workspace-preview-min-width': compactTreeRail
          ? '0px'
          : `${WORKSPACE_PREVIEW_MIN_WIDTH}px`,
      }) as CSSProperties,
  ...
  const openTreeBlankMenu = (event: ReactMouseEvent<HTMLDivElement>) => {
    ...
    if (target?.closest('.workspace-tree__row,.workspace-change,button,input,textarea,select'))
      return;
    event.preventDefault();
    event.stopPropagation();
    setSelectionMenu(null);
  ...
  return (
    ...
    >
        ...
        <section className="workspace-preview">
          <header className="workspace-preview__head">
            <div className="workspace-current-file" aria-label={t('workspace.currentFile')}>
              ...
              <div className="workspace-current-file__text">
                ...
                {previewSubtitle && (
                  <span className="workspace-current-file__path">{previewSubtitle}</span>
                )}
              </div>
          ...
          <div className="workspace-preview__meta">
                  ...
                  <Tooltip label={isLast ? (selectedPath ?? undefined) : dir}>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setFilter('');
                        setOpenDirs(
                          (prev) => new Set([...Array.from(prev), ...breadcrumbDirs, dir]),
                        );
                        void loadDir(dir);
                      }}
                    >
            ...
            {preview && preview.size > 0 && (
              <span className="workspace-preview__size">{formatBytes(preview.size)}</span>
            )}
          </div>
          ...
          >
            {viewMode === 'changed' && scopedChangeRows ? (
              <div className="workspace-change-scope">
                <div className="workspace-change-scope__head">
                  <span className="workspace-change-scope__title">
                    {t('context.sessionChanges')}
                  </span>
                  <span className="workspace-change-scope__meta">
                    {t('context.changedMeta', { count: scopedChangeRows.length })}
                  </span>
                  <Tooltip label={t('workspace.clearChangeScope')}>
                    <button
                      ...
                      onClick={() => {
                        dismissedChangeListRequestIdRef.current =
                          lastChangeListRequestIdRef.current;
                        setScopedChangeRows(null);
                        setSelectedPath(null);
                        setExpandedCommit(null);
                        setCommitDetail(null);
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="workspace-change__body">
                          ...
                          {change.detail && (
                            <span className="workspace-change__detail">{change.detail}</span>
                          )}
                        </span>
                        <span className="workspace-change__meta">
                          <span className="workspace-change__badge workspace-change__badge--git">
                            {change.meta}
                          </span>
                          {change.time && (
                            <span className="workspace-change__badge">{change.time}</span>
                          )}
                        </span>
                      </button>
                    );
                ...
                {sessionChanges && sessionChanges.length > 0 && (
                  <div className="workspace-change-scope">
                    <div className="workspace-change-scope__head">
                      ...
                      <span className="workspace-change-scope__meta">
                        {t('context.changedMeta', { count: sessionChanges.length })}
                      </span>
                    </div>
                    <div className="workspace-change-scope__list">
                      {sessionChanges.map((change) => {
                        ...
                        return (
                          ...
                          >
                            <FileText size={14} />
                            <span className="workspace-change__body">
                              <span className="workspace-change__name">
                                {basename(change.path)}
                              </span>
                              {dir && <span className="workspace-change__path">{dir}</span>}
                              {change.latestPrompt && (
                                <span className="workspace-change__detail">
                                  {change.latestPrompt}
                                </span>
                              )}
                            </span>
                ...
                {loadingHistory ? (
                  ...
                  <div className="workspace-git-history__list">
                    {gitHistory.map((commit) => (
                      <div
                        key={commit.hash}
                        className={`workspace-git-history__item${expandedCommit === commit.hash ? ' workspace-git-history__item--expanded' : ''}`}
                      >
                        <button
                        ...
                        >
                          ...
                          <div className="workspace-git-history__head-bottom">
                            ...
                            <span className="workspace-git-history__date">
                              ...
                              <span className="workspace-git-history__hash">
              ...
              <>
                {preview.truncated && (
                  <div className="workspace-note">{t('workspace.truncated')}</div>
                )}
                {isMarkdown ? (
            ...
            {selectionMenu && (
              <FloatingMenu
                x={selectionMenu.x}
                y={selectionMenu.y}
                estimatedHeight={WORKSPACE_CONTEXT_MENU_SELECTION_HEIGHT}
              >
                <FloatingMenuItems
                  items={[
                    {
      ...
      <section className="workspace-files">
        {showFileTools && (
          <div
            className={`workspace-files__tools${embeddedDockMode ? ' workspace-files__tools--embedded' : ''}`}
          >
            ...
            {showViewTabs && (
              <div
                className="workspace-files__tabs"
                role="tablist"
              ...
              >
                <button
                  className={
                    viewMode === 'files'
                      ? 'workspace-files__tab workspace-files__tab--active'
                      : 'workspace-files__tab'
                  }
        ...
        {scopedFilePaths && (
          <div className="workspace-files__scope">
            ...
            <span className="workspace-files__scope-meta">
              {t('context.readMeta', { count: scopedFilePaths.length })}
            </span>
                ...
                return (
                  ...
                  >
                    {entry.isDir ? (
                      <Folder
                        size={14}
                        className="workspace-tree__icon workspace-tree__icon--dir"
                      />
                    ) : (
                      <FileText size={14} className="workspace-tree__icon" />
                    )}
      ...
      {treeMenu && (
        <FloatingMenu
          ...
          y={treeMenu.y}
          estimatedHeight={
            treeMenu.isDir ? WORKSPACE_CONTEXT_MENU_REF_HEIGHT : WORKSPACE_CONTEXT_MENU_FILE_HEIGHT
          }
        ...
        >
          <FloatingMenuItems
            items={[
              {
                icon: <MessageSquarePlus size={14} />,
                label: treeMenu.isDir
                  ? t('workspace.addFolderReferenceToChat')
                  : t('workspace.addFileReferenceToChat'),
