...
export function Transcript({
  ...
  }, [questions.length, trackQuestions]);
  ...
  const userTurn = useMemo(
    () => new Map(questions.map((question) => [question.id, question.turn])),
    [questions],
  );
  const checkpointsByTurn = useMemo(
    () => new Map(checkpoints.map((checkpoint) => [checkpoint.turn, checkpoint])),
    [checkpoints],
  );
    ...
    for (let i = hotStartIdx; i < items.length; i++) {
      ...
      if (it.kind === 'user') {
        if (current.length > 0) {
          ...
          const isFinal =
            first.kind === 'assistant' && !first.streaming && first.text.trim() !== '';
  ...
  const hotZoneNodes = useMemo<ReactNode[]>(() => {
    ...
    if (stepGroups) {
      ...
      const flushCollapseBatch = () => {
        ...
        const dur = collapseBatch.reduce(
          (ms, it) => ms + (it.kind === 'tool' ? (it.durationMs ?? 0) : 0),
          0,
        );
        out.push(
          <TurnCollapse
      ...
      for (const group of stepGroups) {
        ...
        const hasRunning = nonAssistantItems.some(
          (it) => it.kind === 'tool' && it.status === 'running',
        );
        if (nonAssistantItems.length > 0 && !hasRunning) {
          const dur = nonAssistantItems.reduce(
            (ms, it) => ms + (it.kind === 'tool' ? ((it as ToolItem).durationMs ?? 0) : 0),
            0,
          );
          out.push(
            <TurnCollapse
            ...
            if (it.kind === 'tool') {
              ...
              out.push(
                <ToolCard
                  key={it.id}
                  item={it as ToolItem}
                  subcalls={subcallsByParent.get(it.id)}
                />,
              );
            }
  ...
  return (
    ...
    >
      ...
      <LiveStreamContext.Provider value={live}>
        ...
        <div ref={entranceRef}>{hotZoneNodes}</div>
...
function WarmTurnItems({
  ...
  const flushRO = () => {
    if (roBatch.length === 0) return;
    nodes.push(
      <ReadOnlyBatch key={`rob-${roBatch[0].id}`} items={[...roBatch]} subcalls={subcalls} />,
    );
    roBatch.length = 0;
  };
  ...
  return (
    <div className={`warm-turn${expanded ? ' warm-turn--expanded' : ''}`}>
      ...
      <div ref={contentRef} className="warm-turn__content">
        <div className="warm-turn__body" style={{ display: expanded ? undefined : 'none' }}>
          {children}
        </div>
        {assistantPreview && (
          <div className="warm-turn__assistant" style={{ display: expanded ? 'none' : undefined }}>
            {assistantPreview}
          </div>
        )}
      </div>
...
function TurnCollapse({ items, durationMs, mode, subcalls }: TurnCollapseProps) {
  ...
  const label =
    seconds > 0 ? t('transcript.processedDuration', { s: seconds }) : t('transcript.processed');
  ...
  return (
    <div
      className={`turn-collapse${open ? ' turn-collapse--open' : ''}`}
      data-entrance={displayItems[0]?.id || undefined}
    >
      <button
        type="button"
...
function QuestionJumpBar({
  questions,
  onJump,
}: { questions: QuestionAnchor[]; onJump: (question: QuestionAnchor) => void }) {
  const t = useT();
  ...
  const hoverIdx =
    hovered !== null ? questions.findIndex((question) => question.turn === hovered) : -1;
  const hoveredQuestion =
    hovered !== null ? questions.find((question) => question.turn === hovered) : undefined;
...
function PhaseCard({ text }: { text: string }) {
  return (
    <div className="phase" data-entrance="true">
      <ProcessPhaseIcon size={12} />
      <span>{text}</span>
    </div>
  );
}
...
function CompactionCard({ item }: { item: CompactionItem }) {
  ...
  if (item.pending) {
    return (
      <div className="compaction compaction--pending" data-entrance={item.id}>
        <ProcessCompactIcon size={12} />
        <span>{t('compaction.working')}</span>
      </div>
    );
  }
  return (
    <div className="compaction" data-entrance={item.id}>
      ...
      >
        ...
        <span className="compaction__meta">
          {t('compaction.messages', { n: item.messages })}
