...
export const ToolCard = memo(function ToolCard({
  ...
  const hasArgsOrOutput =
    diffs.length === 0 && (!!effectiveArgs || !!effectiveOutput || item.dataArchived);
  ...
  const summary =
    item.status === 'running'
      ? ''
      : item.summary || summarize(item.name, effectiveArgs, effectiveOutput, item.error);
    ...
    >
      ...
      >
        <span className="tool__label-group">
          ...
          {item.status === 'stopped' && (
            <span className="tool__status-icon tool__status-icon--stopped">—</span>
          )}
      ...
      <div ref={toolBodyRef} className="tool__body">
        ...
        {!shellPreview && hasArgsOrOutput && (
          <>
            {effectiveArgs && (
              <CodeViewer value={pretty(effectiveArgs)} language="json" maxHeight={180} />
            )}
            {effectiveOutput && (
              <>
