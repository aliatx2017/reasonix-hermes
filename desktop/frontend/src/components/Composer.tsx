...
export function Composer({
  ...
  const onKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    const canUseCurrentPromptHistory = () =>
      canUsePromptHistory({
    ...
    if (canUseCurrentPromptHistory()) {
      ...
      void (async () => {
        ...
        if (
          target >= historyEntriesRef.current.length &&
          !(await ensurePromptHistoryIndex(target))
        ) {
          return;
        }
        const next =
        ...
        if (next === -1) {
          ...
          if (
            historyDirection === 'up' &&
            historyEntriesRef.current.length - 1 - next <= PROMPT_HISTORY_PREFETCH_REMAINING
          ) {
  ...
  const composerMetaClass = [
    ...
    planModeOn || goalModeOn || tokenModeOn
      ? 'composer-meta--has-intent-chip'
      : 'composer-meta--no-intent-chip',
  ].join(' ');
        ...
        <div className="composer-access-menu__section">
          ...
          >
            ...
            <span className="composer-access-menu__copy">
              ...
              <span className="composer-access-menu__desc">
                {tokenModeOn ? t('composer.tokenEconomyOnDesc') : t('composer.tokenEconomyDesc')}
              </span>
            </span>
            <span
              className={`composer-intent-switch${tokenModeOn ? ' composer-intent-switch--on' : ''}`}
              aria-hidden="true"
            >
              <span />
            </span>
