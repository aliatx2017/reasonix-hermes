...
export type MessageActionScope =
  | 'fork'
  | 'summ-from'
  | 'summ-upto'
  | 'conversation'
...
export function historyMessagesToItems(
  ...
  const consumedPositionalToolIndexes = new Set(
    Array.from(positionalResults.values(), (result) => result.index),
  );
  ...
  for (let messageIndex = 0; messageIndex < messages.length; messageIndex += 1) {
    ...
    if (m.role === 'assistant') {
      ...
      for (let callIndex = 0; callIndex < toolCalls.length; callIndex += 1) {
        ...
        const positionalResult = tc.id
          ? undefined
          : positionalResults.get(positionalToolResultKey(messageIndex, callIndex));
    ...
    if (m.role === 'tool') {
      if (
        (m.toolCallId && consumedToolIDs.has(m.toolCallId)) ||
        consumedPositionalToolIndexes.has(messageIndex)
      )
        continue;
...
function positionalToolResults(
  messages: HistoryMessage[],
): Map<string, { message: HistoryMessage; index: number }> {
  ...
  for (let messageIndex = 0; messageIndex < messages.length; messageIndex += 1) {
    ...
    for (let callIndex = 0; callIndex < toolCalls.length; callIndex += 1) {
      ...
      while (resultIndex < messages.length) {
        ...
        out.set(positionalToolResultKey(messageIndex, callIndex), {
          message: candidate,
...
function flushPendingUser(s: State): State {
  ...
  return {
    ...
    items: [
      ...s.items,
      { kind: 'user', id: `u${s.seq}`, text: s.pendingUser, createdAt: Date.now() },
    ],
...
function applyEvent(s: State, e: WireEvent): State {
  ...
  switch (e.kind) {
    ...
    case 'reasoning': {
      ...
      const base =
        s.live?.id === id ? s.live : { id, text: '', reasoning: '', reasoningComplete: false };
      const live =
        e.kind === 'text'
          ? {
              ...base,
              text: base.text + delta,
              reasoningComplete: base.reasoning !== '' || base.reasoningComplete,
            }
      ...
      if (idx >= 0) {
        ...
        if (it.kind === 'tool') {
          ...
          const summary =
            ...
            (t.name === it.name && args === it.args ? it.summary : undefined);
          next[idx] = {
            ...it,
            name: t.name,
            args,
    ...
    case 'tool_result': {
      ...
      if (idx >= 0) {
        ...
        if (it.kind === 'tool') {
          ...
          const existing = it;
          const shortArgs =
            existing.args && existing.args.length > 200
              ? existing.args.slice(0, 200) + '…'
              : existing.args;
          const summary = t.err
            ? undefined
            : existing.summary || summarize(existing.name, existing.args, t.output);
          next[idx] = {
            ...existing,
    ...
    case 'usage': {
      ...
      const used =
        e.usage && s.context.window && updateContextGauge ? e.usage.promptTokens : s.context.used;
    ...
    case 'notice':
      return {
        ...
        items: [
          ...s.items,
          { kind: 'notice', id: `n${s.seq}`, level: e.level ?? 'info', text: e.text ?? '' },
        ],
      };
    case 'phase':
      return {
    ...
    case 'compaction_done': {
      ...
      const items =
        at < 0 ? [...s.items, filled] : s.items.map((it, i) => (i === at ? filled : it));
    ...
    case 'steer':
      return {
        ...
        items: [
          ...s.items,
          { kind: 'notice', id: `s${s.seq}`, level: 'info', text: `↪ ${e.text ?? ''}` },
        ],
      };
  ...
  switch (a.type) {
    ...
    case 'send_failed': {
      ...
      for (let i = s.items.length - 1; i >= 0; i--) {
        const it = s.items[i];
        if (it.kind === 'user' && it.text === s.pendingUser) {
          idx = i;
          break;
        }
      }
      const items =
        idx >= 0 ? s.items.map((it, i) => (i === idx ? { ...it, failed: true } : it)) : s.items;
    ...
    case 'backend_status': {
      ...
      const foregroundRunning = foregroundRunningFromRuntimeMeta({
        running: a.running,
        pendingPrompt,
        backgroundJobs,
    ...
    case 'meta':
      return sameMeta(s.meta, a.meta) ? s : { ...s, meta: a.meta };
    case 'context': {
      const sessionTokens =
        typeof a.context.sessionTokens === 'number'
    ...
    case 'checkpoints':
      return { ...s, checkpoints: a.checkpoints };
    case 'message_action_start':
      return { ...s, messageAction: a.action };
    case 'message_action_done':
      return { ...s, messageAction: undefined };
    case 'history': {
    ...
    case 'clearApproval':
      return { ...s, approval: undefined, pendingPrompt: false };
    case 'clearAsk':
      return { ...s, ask: undefined, pendingPrompt: false };
    case 'reset':
      return {
        ...initialState,
        meta: s.meta,
        context: { ...s.context, used: 0, sessionTokens: 0 },
        balance: s.balance,
        effort: s.effort,
        jobs: s.jobs,
...
export function replayPendingPromptsForActiveTab(
  ...
  replay: () => Promise<void> = () => app.ReplayPendingPrompts(),
): void {
...
export function useController() {
  ...
  const loadSessionDataForTab = useCallback(
    async (tabId: string, reset = false) => {
  ...
  const syncActiveTabFromBackend = useCallback(
    async (reset = false): Promise<string | undefined> => {
  ...
  const reconcileTabRuntime = useCallback(
    async (tabId: string): Promise<TabMeta[] | undefined> => {
      const tabs = asArray(await app.ListTabs().catch(() => [] as TabMeta[]));
      const tab = tabs.find((candidate) => candidate.id === tabId);
  ...
  useEffect(() => {
    ...
    const off = onEvent((e) => {
      ...
      if (e.kind === 'turn_done' || e.kind === 'notice') {
        app
          .JobsForTab(targetTabId)
          .then((jobs) => dispatchTo(targetTabId, { type: 'jobs', jobs: asArray(jobs) }))
          .catch(() => {});
      }
    });
  ...
  const sendToTab = useCallback(
    (tabId: string, displayText: string, submitText = displayText) => {
      if (!tabId) return;
  ...
  const runShell = useCallback(
    (command: string) => {
      ...
      dispatchTo(activeTabId, {
        type: 'user',
        text: `!${command}`,
        seq: getOrCreateState(statesRef.current, activeTabId).seq,
      });
  ...
  const notice = useCallback(
    (text: string, level: 'info' | 'warn' = 'info') => {
      if (!activeTabId) return;
  ...
  const approve = useCallback(
    (id: string, allow: boolean, session: boolean, persist: boolean) => {
      if (!activeTabId) return;
  ...
  const setControllerMode = useCallback(
    (mode: Mode): Promise<void> => {
      ...
      return app
        .SetModeForTab(activeTabId, mode)
        .then(() => {
          if (modeHasAutoApproveTools(mode) && activeTabId)
            dispatchTo(activeTabId, { type: 'clearApproval' });
        })
        .catch(() => {});
    },
  ...
  const setCollaborationMode = useCallback(
    async (mode: CollaborationMode): Promise<void> => {
      if (!activeTabId) return;
  ...
  const setToolApprovalMode = useCallback(
    async (mode: ToolApprovalMode): Promise<void> => {
      if (!activeTabId) return;
  ...
  const listSessions = useCallback(
    async (): Promise<SessionMeta[]> =>
      asArray<SessionMeta>(await app.ListSessions().catch(() => [])),
    [],
  );
  const listTrashedSessions = useCallback(
    async (): Promise<SessionMeta[]> =>
      asArray<SessionMeta>(await app.ListTrashedSessions().catch(() => [])),
    [],
  );
  const resumeSession = useCallback(
    async (path: string, tabId?: string) => {
      const targetTabId = tabId || activeTabId;
      if (!targetTabId) return;
      if (tabId) await waitForTabReady(tabId);
      const messages = asArray(
        await (tabId ? app.ResumeSessionForTab(tabId, path) : app.ResumeSession(path)).catch(
          () => [] as HistoryMessage[],
        ),
      );
  ...
  const previewSession = useCallback(
    async (path: string): Promise<HistoryMessage[]> =>
      asArray<HistoryMessage>(await app.PreviewSession(path).catch(() => [])),
    [],
  );
  ...
  const purgeTrashedSession = useCallback(
    (path: string) =>
      app
        .PurgeTrashedSession(path)
        .catch(() => {})
        .finally(() => invalidateCache()),
    [],
  );
  ...
  const refreshMeta = useCallback(async () => {
    ...
    try {
      ...
      dispatchTo(activeTabId, {
        type: 'context',
        context: await app.ContextUsageForTab(activeTabId),
      });
    ...
    async (name: string) => {
      ...
      try {
        ...
        dispatchTo(activeTabId, {
          type: 'local_notice',
          level: 'warn',
          text: t('status.modelSwitchFailed', { err: errorMessage(err) }),
        });
        return;
      }
      try {
    ...
    async (mode: TokenMode) => {
      ...
      try {
        ...
        dispatchTo(activeTabId, {
          type: 'local_notice',
          level: 'warn',
          text: t('status.tokenModeSwitchFailed', { err: errorMessage(err) }),
        });
        return;
      }
      try {
  ...
  const rewind = useCallback(
    async (turn: number, scope: string): Promise<boolean> => {
      const sourceTabId = activeTabId;
      if (!sourceTabId) return false;
      const actionScope = (
        ['fork', 'summ-from', 'summ-upto', 'conversation', 'code', 'both'].includes(scope)
          ? scope
          : 'both'
      ) as MessageActionScope;
      dispatchTo(sourceTabId, {
        type: 'message_action_start',
        action: { turn, scope: actionScope },
      });
      dispatchTo(sourceTabId, {
        type: 'local_notice',
        level: 'info',
        text: messageActionBusyText(actionScope),
      });
      try {
        if (actionScope === 'fork') {
        ...
        const messages = asArray(
          await app.HistoryForTab(sourceTabId).catch(() => [] as HistoryMessage[]),
        );
        dispatchTo(sourceTabId, { type: 'reset' });
        if (messages.length) dispatchTo(sourceTabId, { type: 'history', messages });
        dispatchTo(sourceTabId, {
          type: 'context',
          context: await app.ContextUsageForTab(sourceTabId),
        });
        dispatchTo(sourceTabId, {
          type: 'checkpoints',
          checkpoints: asArray(await app.CheckpointsForTab(sourceTabId)),
        });
        return true;
      } catch {
  ...
  const openProjectTab = useCallback(
    async (workspaceRoot: string, topicId: string): Promise<TabMeta> => {
  ...
  const openGlobalTab = useCallback(
    async (topicId: string): Promise<TabMeta> => {
  ...
  const ensureBlankTab = useCallback(
    async (scope: string, workspaceRoot: string): Promise<TabMeta> => {
