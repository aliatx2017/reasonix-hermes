  ...
  for (const m of messages) {
    ...
    if (m.role === 'assistant') {
      ...
      for (const tc of m.toolCalls ?? []) {
        ...
        const error =
          output.startsWith('[error') || output.startsWith('Error:') ? output : undefined;
        items.push({
          kind: 'tool',
...
export function reducer(s: State, a: Action): State {
  switch (a.type) {
    ...
    case 'backend_status': {
      ...
      if (a.running)
        return { ...s, running: true, turnActive: true, turnStartAt: s.turnStartAt || Date.now() };
      const finalized = s.items.map((it) => {
        if (it.kind === 'assistant' && s.live && it.id === s.live.id)
          return { ...it, text: s.live.text, reasoning: s.live.reasoning, streaming: false };
        if (it.kind === 'assistant' && it.streaming) return { ...it, streaming: false };
...
export function useController() {
  ...
  const restoreSession = useCallback(
    (path: string) => app.RestoreSession(path).catch(() => {}),
    [],
  );
  const purgeTrashedSession = useCallback(
    (path: string) => app.PurgeTrashedSession(path).catch(() => {}),
    [],
  );
  const renameSession = useCallback(
    (path: string, title: string) => app.RenameSession(path, title).catch(() => {}),
    [],
  );
  ...
  const fetchMemory = useCallback(
    (): Promise<MemoryView> =>
      app
        .Memory()
        .catch(() => ({ docs: [], facts: [], scopes: [], storeDir: '', available: false })),
    [],
  );
