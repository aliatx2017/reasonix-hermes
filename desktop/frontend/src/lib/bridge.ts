...
type KnownMissingFromGenerated =
  ...
  | 'HeartbeatGenerateID';
/**/
export type _CheckAppToGen = AssertNever<
  Exclude<Exclude<keyof AppBindings, keyof typeof GeneratedApp>, KnownMissingFromGenerated>
>;
...
export function installWailsNonFileDragErrorSuppression(): () => void {
  ...
  if (!wailsDragSuppressionUninstall) {
    ...
    const hasRecentNativeFileDrag = () =>
      Date.now() - lastNativeFileDragAt <= RECENT_NATIVE_FILE_DRAG_MS;
...
function bridgeBreadcrumb(method: string): string {
  ...
  if (
    /^(Submit|SubmitDisplay|RunShell|Steer|Cancel|Approve|AnswerQuestion|ReplayPendingPrompts)/.test(
      method,
    )
  )
    return `turn ${method}`;
  if (
    /^(SetModel|SetEffort|SetTokenMode|SetDefaultModel|SetPlannerModel|SetSubagentModel|SetSubagentEffort)/.test(
      method,
    )
  )
    return `model ${method}`;
  if (
    /^(SetDesktop|SetCloseBehavior|SetDisplayMode|SetStatusBar|SetExpandThinking|SetAutoPlan|SetReasoningLanguage)/.test(
      method,
    )
  )
    return `settings ${method}`;
  if (
    /^(SaveProvider|AddOfficialProviderAccess|RemoveProviderAccess|DeleteProvider|SetProviderKey|ClearProviderKey|FetchProviderModels|ConnectKey)/.test(
      method,
    )
  )
    return `provider ${method}`;
  if (/^(CheckUpdate|DownloadUpdate|InstallUpdate|ApplyUpdate|OpenDownloadPage)/.test(method))
    return `update ${method}`;
  if (
    /^(AddMCPServer|UpdateMCPServer|RemoveMCPServer|ReconnectMCPServer|ClearMCPServerAuthentication|SetMCPServer)/.test(
      method,
    )
  )
    return `mcp ${method}`;
  if (
    /^(AddSkillPath|RemoveSkillPath|RefreshSkills|SetSkillEnabled|AcceptSkillSuggestion)/.test(
      method,
    )
  )
    return `skill ${method}`;
  if (
    /^(OpenProjectTab|OpenGlobalTab|EnsureBlankTab|SetActiveTab|CloseTab|ReorderTabs|CreateTopic|RenameTopic|DeleteTopic|TrashTopic|RenameProject|RemoveWorkspace|SwitchWorkspace|PickWorkspace)/.test(
      method,
    )
  )
    return `nav ${method}`;
...
function makeMockApp(): AppBindings {
  ...
  let workspaces = freshMock
    ? []
    : [
        '~/projects/joyquant-db',
        '~/projects/joyquant-sys',
        '~/projects/reasonix',
        '~/projects/blade',
      ];
  let mockEffort = 'auto';
  ...
  let capServers: ServerView[] = [
    ...
    },
    {
      ...
      authStatus: 'required',
      authUrl: 'https://mcp.figma.com/mcp',
      tools: 0,
      prompts: 0,
      resources: 0,
  ...
  const capSkills: SkillView[] = [
    ...
    {
      name: 'init',
      description: 'Scaffold a REASONIX.md for this repo',
      scope: 'builtin',
      runAs: 'inline',
      enabled: true,
    },
  ...
  let capSkillRoots: SkillRootView[] = [
    {
      dir: '~/projects/reasonix/.reasonix/skills',
      scope: 'project',
      priority: 1,
      status: 'missing',
      configured: false,
    ...
    },
    {
      ...
      skills: 1,
      skillItems: [
        { name: 'review', description: 'Review the staged diff', scope: 'custom', runAs: 'inline' },
      ],
    },
    {
      dir: '~/.reasonix/skills',
  ...
  const sessions: SessionMeta[] = [
    {
      ...
      preview: 'fix the login bug in auth.go',
      turns: 12,
      createdAt: t0 - 2 * day,
      lastActivityAt: t0 - 3_600_000,
    ...
    },
    {
      ...
      preview: 'refactor the payment module',
      turns: 5,
      createdAt: t0 - 3 * day,
      lastActivityAt: t0 - 6 * 3_600_000,
    ...
    },
    {
      ...
      preview: 'write the README and badges',
      turns: 8,
      createdAt: t0 - 4 * day,
      lastActivityAt: t0 - day - 3_600_000,
  ...
  const settings: SettingsView = {
    ...
    agent: {
      ...
      maxSteps: 0,
      plannerMaxSteps: 12,
      systemPrompt: 'You are Reasonix, a coding agent.',
  ...
  const hookSettings: Record<string, HooksSettingsView> = {
    global: {
      ...
      trusted: true,
      events: hookEvents,
      hooks: [{ event: 'Stop', command: 'echo turn done', description: 'Notify after each turn' }],
    },
    project: {
      scope: 'project',
      path: './.reasonix/settings.json',
    ...
    : [
        {
          ...
          children: [
            {
              key: 'topic_dev_standard',
              kind: 'topic',
              label: `● ${t('mock.topicDevStandard')}`,
              root: '~/projects/joyquant-db',
              topicId: 'topic_dev_standard',
              projectColor: 'blue',
              turns: 18,
              lastActivityAt: mockNow - 8 * 60_000,
              open: true,
              running: runningMock,
            },
            {
              key: 'topic_db_maint',
              kind: 'topic',
              label: t('mock.topicDbMaint'),
              root: '~/projects/joyquant-db',
              topicId: 'topic_db_maint',
              projectColor: 'blue',
              turns: 7,
              lastActivityAt: mockNow - 2 * 60 * 60_000,
            },
            {
              key: 'topic_env',
              kind: 'topic',
              label: t('mock.topicEnv'),
              root: '~/projects/joyquant-db',
              topicId: 'topic_env',
              projectColor: 'blue',
              turns: 3,
              lastActivityAt: mockNow - 26 * 60 * 60_000,
            },
          ],
        },
        {
          ...
          children: [
            {
              key: 'topic_p3b_pd',
              kind: 'topic',
              label: `● ${t('mock.topicP3b')}`,
              root: '~/projects/joyquant-sys',
              topicId: 'topic_p3b_pd',
              projectColor: 'purple',
              turns: 11,
              lastActivityAt: mockNow - 3 * 24 * 60 * 60_000,
              status: runningMock ? 'streaming' : undefined,
            },
            {
              key: 'topic_p3a_pd',
              kind: 'topic',
              label: t('mock.topicP3a'),
              root: '~/projects/joyquant-sys',
              topicId: 'topic_p3a_pd',
              projectColor: 'purple',
              turns: 9,
              lastActivityAt: mockNow - 4 * 24 * 60 * 60_000,
              status: runningMock ? 'thinking' : undefined,
            },
            {
              key: 'topic_hotfix',
              kind: 'topic',
              label: t('mock.topicHotfix'),
              root: '~/projects/joyquant-sys',
              topicId: 'topic_hotfix',
              projectColor: 'purple',
              turns: 4,
              lastActivityAt: mockNow - 5 * 24 * 60 * 60_000,
              status: runningMock ? 'thinking' : undefined,
            },
            {
              key: 'topic_sys_coord',
              kind: 'topic',
              label: t('mock.topicSysCoord'),
              root: '~/projects/joyquant-sys',
              topicId: 'topic_sys_coord',
              projectColor: 'purple',
              turns: 14,
              lastActivityAt: mockNow - 6 * 24 * 60 * 60_000,
              status: runningMock ? 'waiting_confirmation' : undefined,
            },
            {
              key: 'topic_sys_standard',
              kind: 'topic',
              label: t('mock.topicSysStandard'),
              root: '~/projects/joyquant-sys',
              topicId: 'topic_sys_standard',
              projectColor: 'purple',
              turns: 6,
              lastActivityAt: mockNow - 7 * 24 * 60 * 60_000,
              status: 'paused',
            },
            {
              key: 'topic_sys_exception',
              kind: 'topic',
              label: t('mock.topicSysException'),
              root: '~/projects/joyquant-sys',
              topicId: 'topic_sys_exception',
              projectColor: 'purple',
              turns: 2,
              lastActivityAt: mockNow - 8 * 24 * 60 * 60_000,
              status: 'error',
            },
          ],
        },
        {
          ...
          children: [
            {
              key: 'global_topic_product',
              kind: 'global_topic',
              label: t('mock.topicProduct'),
              topicId: 'topic_product',
              turns: 5,
              lastActivityAt: mockNow - 8 * 24 * 60 * 60_000,
            },
            {
              key: 'global_topic_ai',
              kind: 'global_topic',
              label: t('mock.topicAi'),
              topicId: 'topic_ai',
              turns: 8,
              lastActivityAt: mockNow - 10 * 24 * 60 * 60_000,
            },
            {
              key: 'global_topic_lab',
              kind: 'global_topic',
              label: t('mock.topicLab'),
              topicId: 'topic_lab',
              turns: 2,
              lastActivityAt: mockNow - 12 * 24 * 60 * 60_000,
            },
          ],
        },
      ];
  ...
  const projectChildren = (node: ProjectNode): ProjectNode[] =>
  ...
  const setMockProjectPinned = (workspaceRoot: string, pinned: boolean) => {
    const index = mockProjectTree.findIndex(
      (node) => node.kind === 'project' && node.root === workspaceRoot,
    );
    if (index < 0) return;
  ...
  const topicLabel = (topicId: string, fallback: string) =>
    (findMockTopic(topicId)?.label || fallback).replace(/^●\s*/, '');
  ...
  const mockTopicHistory = (topicId: string): HistoryMessage[] => {
    switch (topicId) {
      case 'topic_product':
        return [
          ...
          },
          {
            role: 'assistant',
            content:
              '这是 Global 范围下的 IM 会话。我可以先处理不依赖项目文件的问答、计划和信息整理；需要进入项目时，再由桌面端显式绑定或迁移到项目话题。',
          },
        ];
      case 'topic_ai':
        return [
          ...
          },
          {
            role: 'assistant',
            content:
              '可以。我会先在 Global 范围里整理任务清单；如果某条任务需要读取项目文件，再切到你授权的项目话题处理。',
          },
        ];
      case 'topic_dev_standard':
        return [
          ...
          },
          {
            role: 'assistant',
            content:
              '我可以在桌面端帮你处理代码编写、文件操作、项目分析和问题定位。来自 IM 的请求会进入同一条聊天时间线，桌面端继续承载模型调用、工具执行和上下文管理。',
          },
        ];
      case 'topic_p3b_pd':
        return [
      ...
      case 'topic_p3a_pd':
        return [{ role: 'user', content: '复盘 p3a 的技术方案，先不要写文件，先说明你的判断。' }];
      case 'topic_hotfix':
        return [
          {
            role: 'user',
            content: '检查 post-p3-hotfix 的回归风险，重点看最近的 shell 输出和 git 改动。',
          },
          {
            role: 'assistant',
            content: '',
            reasoning:
              '我先定位最近一次 hotfix 的上下文，然后用只读命令检查状态；左侧保持“思考中”，工具细节在这里展开。',
          },
        ];
      case 'topic_sys_coord':
        return [
          { role: 'user', content: '准备执行 joyquant-sys 的同步脚本，但需要我确认后再运行。' },
          {
            role: 'assistant',
            content: '',
            reasoning: '这个动作会运行脚本并可能刷新本地缓存，所以需要先等用户确认。',
          },
        ];
      case 'topic_sys_standard':
        return [
          { role: 'user', content: '继续制定 SYS 项目开发规范，先停在当前检查点。' },
          {
            role: 'assistant',
            content:
              '已暂停在规范整理阶段。当前保留了目录约定、分支策略和待确认的发布检查项；继续时可以从这里恢复。',
          },
          {
            role: 'notice',
            level: 'info',
            content: '会话已暂停：未继续执行命令，等待用户恢复或切换任务。',
          },
        ];
      case 'topic_sys_exception':
        return [
          ...
          { role: 'assistant', content: '我尝试校验恢复脚本时遇到异常，已停止继续执行。' },
          {
            role: 'notice',
            level: 'warn',
            content: '运行异常：恢复脚本缺少必要环境变量 JOYQUANT_SYS_TOKEN。请补齐配置后重试。',
          },
        ];
      default:
        return [];
    ...
    window.setTimeout(() => {
      void withMockTabScope(tab.id, async () => {
        ...
        if (tab.topicId === 'topic_p3b_pd') {
          const text =
            '我会先把范围拆成三层：目标、依赖、风险。当前已经确认 p3b 的交付边界，接下来补充每个模块的验收口径...';
        ...
        if (tab.topicId === 'topic_p3a_pd') {
          emit({
            kind: 'reasoning',
            text: '我正在对比 p3a 和 p3b 的差异：先看约束，再看变更风险，最后判断是否需要拆成独立任务。\n\n',
          });
          await delay(220);
        ...
        if (tab.topicId === 'topic_hotfix') {
          ...
          emit({
            ...
            tool: {
              id,
              name: 'bash',
              args: JSON.stringify({ command: 'git status --short && npm test' }),
              readOnly: true,
            },
          });
          await delay(180);
        ...
        if (tab.topicId === 'topic_sys_coord') {
          ...
          emit({
            kind: 'reasoning',
            text: '我已经准备好执行同步脚本，但这个操作会影响本地 workspace，需要用户确认。',
          });
          await delay(160);
          emit({
  ...
  const mockModelLabel = (ref: string): string =>
    mockModelCatalog.find((model) => model.ref === mockModelRef(ref))?.model ??
    ref.split('/').pop() ??
    ref;
  ...
  return {
    ...
    async Submit(input) {
      ...
      if (goalMatch) {
        ...
        if (!arg || lowered === 'status') {
          emit({
            kind: 'notice',
            level: 'info',
            text: active?.goal ? `goal: ${active.goal}` : 'goal: none',
          });
          emitMockTurnDone();
          return;
        }
        if (['clear', 'off', 'stop', 'done'].includes(lowered)) {
          mockTabs = mockTabs.map((tab) =>
            tab.active
              ? { ...tab, goal: '', goalStatus: 'stopped', collaborationMode: 'normal' }
              : tab,
          );
        ...
        mockTabs = mockTabs.map((tab) =>
          tab.active
            ? { ...tab, goal: arg, goalStatus: 'running', collaborationMode: 'goal' }
            : tab,
        );
        ...
        mockTabs = mockTabs.map((tab) =>
          tab.active
            ? { ...tab, goal: '', goalStatus: 'complete', collaborationMode: 'normal' }
            : tab,
        );
      ...
      if (
        trimmedInput === '/approve-preview' ||
        trimmedInput === 'approve preview' ||
        trimmedInput === 'approve预览'
      ) {
        pendingApprovalPreview = true;
        await delay(250);
        if (cancelled) return;
          ...
          tool: {
            ...
            args: JSON.stringify({
              todos: [
                ...
                {
                  content: t('mock.todo2'),
                  activeForm: t('mock.todo2ActiveForm'),
                  status: 'in_progress',
                },
      ...
      if (
        ...
        emit({
          kind: 'notice',
          level: 'info',
          text: 'Loaded project instructions from AGENTS.md.',
        });
        await delay(120);
        emit({
          kind: 'notice',
          level: 'warn',
          text: 'Network access is enabled; external results may change over time.',
        });
        await delay(120);
        ...
        emit({
          ...
          compaction: {
            ...
            messages: 6,
            summary:
              'Preserved the active task, relevant files, and UI decisions while trimming earlier exploratory context.',
          },
        });
    ...
    async RunShell(command) {
      ...
      emit({
        kind: 'tool_dispatch',
        tool: { id, name: 'bash', args: JSON.stringify({ command }), readOnly: false },
      });
      await delay(200);
      if (cancelled) return;
      emit({
        kind: 'tool_progress',
        tool: { id, name: 'bash', output: `$ ${command}\n(mock output)\n`, readOnly: false },
      });
      await delay(100);
      if (cancelled) return;
      emit({
    ...
    async AnswerQuestion(_id, answers) {
      ...
      const summary = answers
        .map(
          (answer) =>
            `${answer.questionId}: ${(answer.selected ?? []).join(', ') || '(no answer)'}`,
        )
        .join('\n');
    ...
    async SetCollaborationModeForTab(tabID, mode) {
      ...
      mockTabs = mockTabs.map((tab) => {
        ...
        return {
          ...
          mode: modeWithPlan(
            modeWithAutoApproveTools(normalizeMode(tab.mode), toolMode === 'yolo'),
            next === 'plan',
          ),
        };
      });
    ...
    async HistoryForTab(tabID?: string) {
      const tab =
        mockTabs.find((item) => item.id === tabID) ?? mockTabs.find((item) => item.active);
      if (tab?.topicId) {
    ...
    async PreviewSession(path: string) {
      const s =
        sessions.find((x) => x.path === path) ?? trashedSessions.find((x) => x.path === path);
      return [
        ...
        },
        {
          role: 'notice',
          level: 'info',
          content: 'Preview mode keeps the active conversation untouched.',
        },
        {
          role: 'compaction',
          content: '',
          trigger: 'manual',
          messages: 3,
          summary: 'Mock preview preserved the latest task, tool result, and answer summary.',
        },
      ];
    },
      ...
      const entries: PromptHistoryEntry[] = [
        {
          ...
          at: Date.now() - 60000,
          sessionPath: '/mock/sessions/arch.jsonl',
          turn: 0,
        },
        {
        ...
        },
        {
          ...
          at: Date.now() - 300000,
          sessionPath: '/mock/sessions/general.jsonl',
          turn: 0,
        },
      ];
    ...
    async PickWorkspace() {
      ...
      return mockSwitchWorkspace(
        cwd.endsWith('another-project') ? '~/projects/reasonix' : '~/projects/another-project',
      );
    },
    ...
    async Meta() {
      ...
      const toolApprovalMode = normalizeToolApprovalMode(
        active?.toolApprovalMode,
        active ? normalizeMode(active.mode) : 'normal',
    ...
    async MetaForTab(tabID) {
      const tab =
        mockTabs.find((item) => item.id === tabID) ??
        mockTabs.find((item) => item.active) ??
        mockTabs[0];
      const toolApprovalMode = normalizeToolApprovalMode(
        tab?.toolApprovalMode,
        tab ? normalizeMode(tab.mode) : 'normal',
        settings.autoApproveTools,
      );
      const autoApproveTools = toolApprovalMode === 'yolo';
      const collaborationMode = normalizeCollaborationMode(
    ...
    async Commands() {
      return [
        {
          name: 'new',
          description: 'start new session; save transcript',
    ...
    async UpdateMCPServer(name: string, input: MCPServerInput) {
      capServers = capServers.map((s) => {
        ...
        const nextStatus =
          s.status === 'disabled' ? 'disabled' : connected ? 'connected' : 'deferred';
        const nextTools =
          nextStatus === 'connected' ? s.tools || (input.transport === 'stdio' ? 3 : 5) : 0;
        return {
          ...s,
          transport: input.transport,
          status: nextStatus,
          ...
          error: undefined,
          authStatus:
            nextStatus !== 'connected' && input.transport !== 'stdio' ? 'possible' : undefined,
          authUrl:
            nextStatus !== 'connected' && input.transport !== 'stdio' ? input.url : undefined,
        };
      });
    },
      ...
      return [
        {
          ...
          message: 'Mock commit message for ' + path,
        },
      ];
    },
        ...
        skills: [
          {
            ...
            description:
              'Review or update a Reasonix GitHub PR, address feedback, verify, and publish safely.',
            scope: 'project',
            ...
            evidence: [
              'mock-pr-session: 提交到pr，并更新内容',
              'mock-review-session: 解决该pr下机器人提出来的问题',
            ],
          },
        ],
    ...
    async AddOfficialProviderAccess(kind: string, key: string) {
      ...
      if (i >= 0)
        settings.providers[i] = {
          ...settings.providers[i],
          ...next,
          keySet: next.keySet || settings.providers[i].keySet,
        };
    ...
    async FetchProviderModels(p: ProviderView) {
      ...
      if (p.baseUrl.includes('mimo') || p.baseUrl.includes('xiaomimimo'))
    ...
    async SetBotSecret(envName: string, _value: string) {
      ...
      settings.bot.connections = settings.bot.connections.map((connection) => ({
        ...connection,
        credential:
          connection.credential.appSecretEnv === name || connection.credential.tokenEnv === name
    ...
    async BotRuntimeStatus() {
      const qqRunning =
        settings.bot.qq.enabled && settings.bot.qq.appId.trim() && settings.bot.qq.secretSet;
      const runningConnections =
        (qqRunning ? 1 : 0) +
        settings.bot.connections.filter(
      ...
      return {
        ...
        message:
          settings.bot.enabled && runningConnections > 0
            ? `${runningConnections} bot connection(s) running`
            : 'bot runtime is not started',
    ...
    async StartBotConnectionInstall(provider: string, domain: string) {
      ...
      const normalizedDomain =
        normalizedProvider === 'weixin' ? 'weixin' : domain === 'lark' ? 'lark' : 'feishu';
      return {
        ok: true,
    ...
    async PollBotConnectionInstall(installID: string) {
      ...
      settings.bot.connections = [
        ...settings.bot.connections.filter((c) => c.id !== connection.id),
        connection,
      ];
    ...
    async DiagnoseBotConnection(id: string) {
      ...
      return connection
        ? {
            ...
            status: connection.enabled ? 'ok' : 'disabled',
            message: connection.enabled ? '连接配置已保存。' : '连接已保存但未启用。',
            messageId: '',
            phase: 'config',
            code: connection.enabled ? 'config_ok' : 'connection_disabled',
            reportKind: '',
            reportDetail: '',
            occurredAt,
          }
        : {
            ...
            reportKind: 'bot',
            reportDetail: JSON.stringify({
              schemaVersion: 2,
              kind: 'bot',
              source: 'bot.runtime',
              label: 'bot.mock.config',
              message: 'mock missing bot connection',
              errorType: 'BotConnectionDiagnostic',
              errorMessage: 'bot connection record was not found',
              topFrame: 'bot.config',
              occurredAt,
            }),
            occurredAt,
    ...
    async TestBotConnection(id: string, target?: string) {
      ...
      if (target?.trim())
        return {
          ...diag,
          message: `Mock test sent to ${target.trim()}`,
    ...
    async MigrateDesktopPreferences(language: string, theme: string, style: string) {
      if (!settings.desktopLanguage)
        settings.desktopLanguage =
          language === 'en' || language === 'zh' || language === 'zh-TW' ? language : '';
      if (!settings.desktopTheme && !settings.desktopThemeStyle) {
        settings.desktopTheme = theme === 'auto' || theme === 'light' ? theme : 'dark';
        settings.desktopThemeStyle = style;
      }
    },
    async SetAgentParams(
      temperature: number,
      maxSteps: number,
      plannerMaxSteps: number,
      systemPrompt: string,
    ...
    // ── Heartbeat mock ──
    async HeartbeatListTasks() {
      return [];
    },
    async HeartbeatReloadTasks() {
      return [];
    },
    async HeartbeatSaveTasks(_tasks: unknown) {},
    async HeartbeatTriggerNow(_id: string) {},
    async HeartbeatGenerateID() {
    ...
    async ConnectDiscordBot(_token: string): Promise<DiscordConnectResult> {
      return {
        ...
        connection: {
          ...
          credential: {
            appId: '',
            appSecretEnv: 'DISCORD_BOT_TOKEN',
            accountId: '',
    ...
    async ConstitutionHealth(): Promise<ConstitutionHealthView> {
      return {
        loaded: true,
        path: '.reasonix/constitution.json',
        version: 1,
        rules: [],
        principles: ['Evidence-first reasoning', 'Mutable scratch'],
        constraints: ['No hardcoded switches'],
        status: 'ok',
      };
    },
    ...
    async LearnedPatterns(): Promise<[LearnedPatternView[], LearnedTrajectoryView[]]> {
      return [
        [
          {
            name: 'edit-then-test',
            trigger: 'after editing Go files',
            action: 'run go build ./...',
            confidence: 4,
            draft: '[edit-then-test]\ndescription = …',
          },
        ],
    ...
    async EnsureBlankTab(scope: string, workspaceRoot: string) {
      ...
      return targetScope === 'global'
        ? this.OpenGlobalTab(topic.id)
        : this.OpenProjectTab(targetRoot, topic.id);
    },
    ...
    async ContextPanel(_tabID: string) {
      ...
      return {
        ...
        changedFiles: [
          {
            path: t('mock.changedFile1Path'),
            sources: ['session'],
            gitStatus: 'modified',
            turns: [5, 6],
            latestPrompt: t('mock.changedFile1Prompt'),
            latestTime: now - 2 * 60 * 1000,
          },
          {
            path: t('mock.changedFile2Path'),
            sources: ['session'],
            gitStatus: 'added',
            turns: [6],
            latestPrompt: t('mock.changedFile2Prompt'),
            latestTime: now - 60 * 1000,
          },
        ],
      };
    },
