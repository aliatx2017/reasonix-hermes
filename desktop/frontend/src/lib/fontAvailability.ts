...
const UI_FONT_CANDIDATES: Record<
  Exclude<FontFamily, 'system' | 'custom'>,
  { platforms: TypographyPlatform[]; fonts: string[] }
> = {
  yahei: {
    platforms: ['windows'],
...
const MONO_FONT_CANDIDATES: Record<
  Exclude<MonoFontFamily, 'system' | 'custom'>,
  { platforms: TypographyPlatform[]; fonts: string[] }
> = {
  cascadia: {
    platforms: ['windows'],
