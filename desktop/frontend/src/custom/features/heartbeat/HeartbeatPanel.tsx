...
export function HeartbeatPanel({ open, onClose, startNew, onOpenTopic }: HeartbeatPanelProps) {
  ...
  const loadTasks = useCallback(async () => {
    setLoading(true);
    try {
      const [taskList, wsList] = await Promise.all([heartbeatListTasks(), app.ListWorkspaces()]);
      setTasks(taskList);
      const map: Record<string, string> = {};
      if (wsList) {
        wsList.forEach((ws) => {
          if (ws.path) map[ws.path] = ws.name;
        });
      }
      setWorkspaceMap(map);
    } catch {
          ...
          <div className="heartbeat-modal__body">
            <div className="heartbeat-toolbar">
              <div className="heartbeat-toolbar__search">
                ...
                {searchQuery && (
                  <button
                    className="heartbeat-toolbar__search-clear"
                  ...
                  <div className="heartbeat-filter-menu__list" role="listbox">
                    {(['all', 'enabled', 'disabled'] as const).map((key) => (
                      ...
                      >
                        <span>
                          {key === 'all'
                            ? t('heartbeat.filterAll' as any)
                            : key === 'enabled'
                              ? t('heartbeat.filterEnabled' as any)
                              : t('heartbeat.filterDisabled' as any)}
                        </span>
                        {statusFilter === key && (
                          <Check size={12} className="heartbeat-filter-menu__check" />
                        )}
                      </button>
                    ))}
                  </div>
              ...
              <button
                className="heartbeat-toolbar__btn heartbeat-toolbar__btn--primary"
            ...
            {(() => {
              ...
              const scopeLabel = (task: HeartbeatTask): string => {
                ...
                return (
                  workspaceMap[task.workspaceRoot] ||
                  task.workspaceRoot.split('/').pop() ||
                  task.workspaceRoot
                );
              };
      ...
      <div className="heartbeat-card__head">
        ...
        <span className="heartbeat-card__title">
          <button type="button" className="heartbeat-card__title-btn" onClick={onEdit}>
            <span className="heartbeat-card__title-text">
              {task.title || t('heartbeat.untitled')}
            </span>
...
function CycleEditor({
  ...
  const t = useT();
<<<<<<< HEAD
  const cycleMatch = draft.interval.match(
    /^(\d+)[smh]\|(daily|weekly|biweekly|monthly|yearly)(?::([^@]*))?(?:@(\d{2}:\d{2}))?$/,
  );
  ...
  const [selectedDays, setSelectedDays] = useState<string[]>(cycleDays ? cycleDays.split(',') : []);
...
<<<<<<< HEAD
  ...
  const onCycleTypeChange = useCallback(
    (ct: string) => {
      setCycleType(ct);
      const days: string[] = [];
...
>>>>>>> upstream/main-v2
  ...
  const onMonthDayChange = useCallback(
    (d: string) => {
      setMonthDay(d);
  ...
  const onYearDayChange = useCallback(
    (d: string) => {
      setYearDay(d);
...
>>>>>>> upstream/main-v2
  ...
  const selectedWorkspace =
    draft.scope === 'project' && draft.workspaceRoot
...
>>>>>>> upstream/main-v2
      ...
      <div className="heartbeat-editor__field">
        <label>
          {t('heartbeat.fieldScope')}{' '}
          <span className="heartbeat-editor__optional">{t('heartbeat.optional')}</span>
        </label>
                ...
                {workspaces.length === 0 ? (
                  ...
                  workspaces.map((ws) => (
                    ...
                    >
                      {ws.name}
                      {ws.current && (
                        <span className="heartbeat-project-menu__current">
                          {t('heartbeat.currentWorkspace')}
                        </span>
                      )}
                    </button>
                  ))
...
>>>>>>> upstream/main-v2
        ...
        {freqType === 'cycle' ? (
          <CycleEditor draft={draft} setDraft={set} />
        ) : (
...
>>>>>>> upstream/main-v2
      ...
      <div className="heartbeat-editor__actions">
        {!isNew && !confirmingDelete && (
          <button
            className="heartbeat-btn heartbeat-btn--danger"
