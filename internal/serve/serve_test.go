...
// mustNew wraps New for tests, panicking on auth-init error (never happens with
// well-formed ServeConfig values; crypto/rand never fails in test environments).
func mustNew(ctrl control.SessionAPI, bc *Broadcaster, cfg config.ServeConfig) *Server {
	s, err := New(ctrl, bc, cfg)
	if err != nil {
...
func TestServeSubmitRunsAndBroadcastsTurnDone(t *testing.T) {
	...
	srv := httptest.NewServer(mustNew(ctrl, bc, config.ServeConfig{}).Handler())
	defer srv.Close()
